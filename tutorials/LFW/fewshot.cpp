...
int main(int argc, char const* argv[])
{
    ...
    Tensorholder<float>* label = new Tensorholder<float>(1, BATCH, 1, 1, NUMBER_OF_CLASS,
                                                         "label"); // NUMBER_OF_CLASS is not used

    // ======================= Creating net ===================
    printf("Creating neural net...");
    fflush(stdout);
    NeuralNetwork<float>* net =
        new my_FaceNet<float>(x, label, NUMBER_OF_CLASS); // NUMBER_OF_CLASS is not used
    printf("Done.\n");
    fflush(stdout);
    ...
    LFWDataset<float>* train_dataset =
        new LFWDataset<float>("./data", "ETRI_LFW_train", NUMBER_OF_CLASS, transform);
    LFWDataset<float>* test_dataset =
        new LFWDataset<float>("./data", "ETRI_LFW_test", NUMBER_OF_CLASS + 1, transform);
    ...
    std::vector<std::string> before_classname = {
        "Bill_Gates",  "George_Robertson", "Gray_Davis",     "Halle_Berry",  "Hans_Blix",
        "Hugo_Chavez", "Igor_Ivanov",      "Jacques_Chirac", "Jean_Charest", "Jennifer_Aniston"};
    std::vector<std::string> after_classname = {
        "Bill_Gates",   "George_Robertson", "Gray_Davis",       "Halle_Berry",
        "Hans_Blix",    "Hugo_Chavez",      "Igor_Ivanov",      "Jacques_Chirac",
        "Jean_Charest", "Jennifer_Aniston", "Jeremy_Greenstock"};
    std::ifstream fin;
    std::ofstream fout;
    ...
    GetReferenceSamples(train_dataset, INPUT_DIM, NUMBER_OF_CLASS, SAMPLE_PER_CLASS, vRefSamples,
                        vRefLabels);
    FewShotClassifier few_shot(INPUT_DIM, FEATURE_DIM, before_classname, net, vRefSamples.size(),
                               &vRefLabels[0], &vRefSamples[0], BATCH);
    ...
    char inputFile[256];
    sprintf(inputFile, "./data/ETRI_LFW_test/Jeremy_Greenstock/Jeremy_Greenstock_%04d.jpg",
            testIdx);
    ...
    // GetReferenceSamples(test_dataset, INPUT_DIM, NUMBER_OF_CLASS + 1, SAMPLE_PER_CLASS,
    // test_vRefSamples, test_vRefLabels); FewShotClassifier test_few_shot(INPUT_DIM, FEATURE_DIM,
    // after_classname, net, test_vRefSamples.size(), &test_vRefLabels[0], &test_vRefSamples[0],
    // BATCH);
