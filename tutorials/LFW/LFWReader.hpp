...
public:
    ...
    int CreateImageListOfEachClass()
    {
        ...
        if (m_isTrain)
        {
            ...
            {
                sprintf(filePath, "%s/%s/%s/list.txt", m_path.c_str(), m_dirOfTrainImage.c_str(),
                        m_className[classNum].c_str());
                // const char *cstr = filePath.c_str();
            ...
            for (int classNum = 0; classNum < NUMBER_OF_CLASS; classNum++)
            {
                ...
                // + "/list.txt"; // check with printf const char *cstr = filePath.c_str();
        ...
        else
        {
            for (int classNum = 0; classNum < NUMBER_OF_CLASS; classNum++)
            {
                sprintf(filePath, "%s/%s/%s/list.txt", m_path.c_str(), m_dirOfTestImage.c_str(),
                        m_className[classNum].c_str());
