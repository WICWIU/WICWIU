...
class LFWDatasetForSample : public Dataset<DTYPE>{
private:
    int m_numOfImg;
    int m_curImg;
...
public:
    LFWDatasetForSample(std::string rootPath, std::string dataPath, int useClassNum, vision::Compose *transform) {
        m_rootPath   = rootPath;
        m_dataPath   = dataPath;
    ...
    virtual std::vector<Tensor<DTYPE> *>* GetData(int requestedLabel);
...
template<typename DTYPE> void LFWDatasetForSample<DTYPE>::Alloc()  {}

template<typename DTYPE> void LFWDatasetForSample<DTYPE>::Delete() {}

//
/*
template<typename DTYPE> std::vector<Tensor<DTYPE> *> *LFWDatasetForSample<DTYPE>::GetData(int requestedLabel) {
    std::vector<Tensor<DTYPE> *> *result = new std::vector<Tensor<DTYPE> *>(0, NULL);


    do{
      m_curImg = (m_curImg != m_numOfImage -1) ? m_curImg + 1 : 0;
      if((int)m_aLable[m_curImg] == requestedLabel) break;
    }while(TRUE);
    ...
    // image load
#ifdef __TURBOJPEG__
    this->AllocImageBuffer(m_curImg, imgWrp);
    ...
    (*label)[m_aLable[m_curImg]] = (DTYPE)1;
...
template<typename DTYPE> std::vector<Tensor<DTYPE> *> *LFWDatasetForSample<DTYPE>::GetData(int idx) {
    std::vector<Tensor<DTYPE> *> *result = new std::vector<Tensor<DTYPE> *>(0, NULL);
    ...
    do{
      m_curImg = (m_curImg != m_numOfImage -1) ? m_curImg + 1 : 0;
      if((int)m_aLable[m_curImg] == idx) break;
    }while(TRUE);
...
template<typename DTYPE> int LFWDatasetForSample<DTYPE>::GetLength() {
    return m_numOfImage;
}

template<typename DTYPE> void LFWDatasetForSample<DTYPE>::CheckClassList() {
    // mnt/ssd/Data/ImageNet/synset_words.txt
    std::string filePath = m_rootPath + "/label_word.txt";
...
template<typename DTYPE> void LFWDatasetForSample<DTYPE>::CreateImageListOfEachClass() {
...
template<typename DTYPE> void LFWDatasetForSample<DTYPE>::AllocImageBuffer(int idx, ImageWrapper& imgWrp) {
    int   width, height;
    FILE *jpegFile         = NULL;
...
template<typename DTYPE> Tensor<DTYPE> *LFWDatasetForSample<DTYPE>::Image2Tensor(ImageWrapper& imgWrp, int doValueScaling) {
...
template<typename DTYPE> void LFWDatasetForSample<DTYPE>::Tensor2Image(std::string filename, Tensor<DTYPE> *imgTensor, int doValuerScaling) {
