    ...
    int Alloc()
    {
        if (m_isTrain)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                m_aaTrainImageSrcs[i] =
                    (unsigned char*)malloc(sizeof(unsigned char) * SIZE_OF_SINGLE_DATA_FILE);

                fread(m_aaTrainImageSrcs[i], sizeof(unsigned char), SIZE_OF_SINGLE_DATA_FILE,
                      pFile);
...
public:
    ...
    int GetNormalizationMode() { return m_isNormalizePerChannelWise; }

    float* GetSetOfMean() { return m_aMean; }

    float* GetSetOfStddev() { return m_aStddev; }
    ...
    {
        int limitOfCropPos =
            LEGNTH_OF_WIDTH_AND_HEIGHT + (2 * m_padding) - m_lengthOfWidthAndHeight + 1;
            ...
            do
            {
                ...
                if (m_useRandomCrop)
                {
                    ...
                    random_shuffle(m_shuffledListForCrop.begin(), m_shuffledListForCrop.end(),
                                   CIFAR10Reader<DTYPE>::random_generator);
                }
                ...
                if (m_useRandomHorizontalFlip)
                {
                    ...
                    srand(unsigned(time(0)));
                    random_shuffle(m_shuffledListForFlip.begin(), m_shuffledListForFlip.end(),
                                   CIFAR10Reader<DTYPE>::random_generator);
                }
    ...
    Tensor<DTYPE>* Image2Tensor(int srcNum, int imgNum /*Address of Image*/)
    {
        ...
        Tensor<DTYPE>* temp =
            Tensor<DTYPE>::Zeros(1, 1, 1, 1, numOfChannel * heightOfTensor * widthOfTensor);
        ...
        for (int channelNum = 0; channelNum < numOfChannel; channelNum++)
        {
            for (int heightIdx = 0; heightIdx < heightOfImg; heightIdx++)
            {
                ...
                {
                    idx = channelNum * planeSizeOfTensor + (heightIdx + padding) * widthOfTensor +
                          (widthIdx + padding);
                    (*temp)[idx] = (DTYPE)*ip / 255;
                    // printf("%d = %d * %d + (%d + %d) * %d + (%d + %d) = ", idx, channelNum,
                    // planeSizeOfTensor, heightIdx, padding, widthOfTensor, widthIdx, padding);
                    // std::cout << "idx : " << idx << '\n';
                    // int temp_;
                    // std::cin >> temp_;
                }
        ...
        if (m_isNormalizePerChannelWise)
        {
            ...
            for (int channelNum = 0; channelNum < numOfChannel; channelNum++)
            {
                for (int heightIdx = 0; heightIdx < heightOfImg; heightIdx++)
                {
                    ...
                    {
                        idx = channelNum * planeSizeOfTensor +
                              (heightIdx + padding) * widthOfTensor + (widthIdx + padding);
                ...
                for (int heightIdx = 0; heightIdx < heightOfImg; heightIdx++)
                {
                    for (int widthIdx = 0; widthIdx < widthOfImg; widthIdx++)
                    {
                        idx = channelNum * planeSizeOfTensor +
                              (heightIdx + padding) * widthOfTensor + (widthIdx + padding);
                        idxOfMeanAdnStddev =
                            channelNum * planeSizeOfTensor + heightIdx * widthOfImg + widthIdx;
                        (*image)[idx] -= m_aMean[channelNum];
                        (*image)[idx] /= m_aStddev[channelNum];
                    }
                }
    ...
    Tensor<DTYPE>* RandomCrop(Tensor<DTYPE>* srcImage)
    {
        ...
        Tensor<DTYPE>* cropedImg =
            Tensor<DTYPE>::Zeros(1, 1, 1, 1, numOfChannel * heightOfImg * widthOfImg);
        ...
        for (int channelNum = 0; channelNum < numOfChannel; channelNum++)
        {
            for (int rowNum = 0; rowNum < heightOfImg; rowNum++)
            {
                for (int colNum = 0; colNum < widthOfImg; colNum++)
                {
                    ...
                    // << '\n'; std::cout << "cropedImg : " << cropedImg->GetShape() << "idx : " <<
                    // idx << '\n';
