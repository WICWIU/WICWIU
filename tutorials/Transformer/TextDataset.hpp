...
public:
    ...
    virtual std::vector<Tensor<DTYPE> *> *GetData(int idx) {
        ...
        Tensor<DTYPE> *encoderInput = Tensor<DTYPE>::Zeros(1, 1, 1, 1, maxLength);
        Tensor<DTYPE> *decoderInput = Tensor<DTYPE>::Zeros(1, 1, 1, 1, maxLength);
        Tensor<DTYPE> *label        = Tensor<DTYPE>::Zeros(1, 1, maxLength, 1, tgtVocab->GetNumberofUniqueWords());
        ...
        if (splitted_src.size() > maxLength || splitted_tgt.size() > maxLength) {
            cout << splitted_src.size() << "/" << splitted_tgt.size() << "/" << maxLength << '\n';
            exit(1);
        }
...
public:
    ...
    virtual std::vector<Tensor<DTYPE> *> *GetData(int idx) {
        ...
        Tensor<DTYPE> *encoderInput = Tensor<DTYPE>::Zeros(srcVocab->GetMaxSentenceLength() + 1, 1, 1, 1, 1);
        Tensor<DTYPE> *decoderInput = Tensor<DTYPE>::Zeros(tgtVocab->GetMaxSentenceLength() + 1, 1, 1, 1, 1);
        Tensor<DTYPE> *label        = Tensor<DTYPE>::Zeros(tgtVocab->GetMaxSentenceLength() + 1, 1, 1, 1, tgtVocab->GetNumberofUniqueWords());
    ...
    void MakePretrainedEmbedding() {            
        std::cout << " ---------------- MakePretrainedEmbedding ----------------------" << '\n';
        ...
        while (fin.peek() != EOF) {
            ...
            vector<string> wordAndEmbedding = this->SplitBy(buffer, ' ');
        ...
        while (fin_en.peek() != EOF) {
            ...
            vector<string> wordAndEmbeddingTgt = this->SplitBy(buffer, ' ');
        ...
        std::cout << " ---------------- MakePretrainedEmbedding End----------------------" << '\n';
    }
