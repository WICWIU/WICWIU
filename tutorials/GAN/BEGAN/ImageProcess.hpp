...
public:
    Normalize(std::initializer_list<float> mean_value, std::initializer_list<float> stddev_value)
        : list_mean(mean_value), list_stddev(stddev_value)
    {
    }
            ...
            for (int newx = 0; newx < oldWidth; newx++)
            {
                for (int c = 0; c < channel; c++)
                {
                    newImgBuf[newy * oldWidth * channel + newx * channel + c] =
                        (oldImgBuf[newy * oldWidth * channel + newx * channel + c] - list_mean[c]) /
                        list_stddev[c];
                }
            }
        }
        ...
        for (int newy = 0; newy < newHeight; newy++)
        {
            for (int newx = 0; newx < newWidth; newx++)
            {
                for (int c = 0; c < channel; c++)
                {
                    ...
                    else if ((newx >= (newWidth - p_value) && newx <= newWidth - 1) ||
                             (newy >= (newHeight - p_value) && newx <= newHeight - 1))
                        newImgBuf[newy * newWidth * channel + newx * channel + c] = 0;

                    else
                    {
                        newImgBuf[newy * newWidth * channel + newx * channel + c] =
                            oldImgBuf[(newy - p_value) * oldWidth * channel +
                                      (newx - p_value) * channel + c];
                    }
                }
            }
        }
