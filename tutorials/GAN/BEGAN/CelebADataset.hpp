...
class CelebADataset : public Dataset<DTYPE>
{
private:
    /* data */
    string m_imagePath;
...
public:
    ...
    void Tensor2Image(Tensor<DTYPE>* temp, const char* FILENAME, int colorDim, int batch,
...
template <typename DTYPE>
CelebADataset<DTYPE>::CelebADataset(string pImagePath)
{
...
template <typename DTYPE>
CelebADataset<DTYPE>::~CelebADataset()
{
...
template <typename DTYPE>
void CelebADataset<DTYPE>::Alloc()
{
}

template <typename DTYPE>
void CelebADataset<DTYPE>::Dealloc()
{
}

template <typename DTYPE>
void CelebADataset<DTYPE>::initImageNames()
{
    string temp = "";
...
template <typename DTYPE>
std::vector<Tensor<DTYPE>*>* CelebADataset<DTYPE>::GetData(int idx)
{
    ...
    Tensor<DTYPE>* temp = Tensor<DTYPE>::Zeros(
        1, 1, 3, LENGTH_64, LENGTH_64); // multi channel일 떄 col에 체널수도 곱해줘야함.
        ...
        for (int co = 0; co < width; co++)
        {
            for (int ch = 0; ch < NUMBER_OF_CHANNEL; ch++)
            {
                ...
                (*temp)[Index5D(temp->GetShape(), 0, 0, ch, ro, co)] =
                    imgBuf[ro * width * NUMBER_OF_CHANNEL + co * NUMBER_OF_CHANNEL + ch] / 255.0 *
                        2 -
                    1;
                ...
                //     * 2 - 1
                //     + imgBuf[ro * width * NUMBER_OF_CHANNEL + co * NUMBER_OF_CHANNEL + 2] / 255.0
                //     * 2 - 1) / 3;

                // (*temp)[Index5D(temp->GetShape(), 0, 0, 0, ro, co)]
                //      = (imgBuf[ro * width * NUMBER_OF_CHANNEL + co * NUMBER_OF_CHANNEL] / 255.0
                //      + imgBuf[ro * width * NUMBER_OF_CHANNEL + co * NUMBER_OF_CHANNEL + 1] /
                //      255.0
                //      + imgBuf[ro * width * NUMBER_OF_CHANNEL + co * NUMBER_OF_CHANNEL + 2] /
                //      255.0) / 3;
            }
        }
    }
    ...
    temp->ReShape(1, 1, 1, 1,
                  3 * LENGTH_64 * LENGTH_64); // multi channel일 떄 col에 체널수도 곱해줘야함.
...
template <typename DTYPE>
int CelebADataset<DTYPE>::UseNormalization(int isNormalizePerChannelWise, float* mean,
                                           float* stddev)
{
...
template <typename DTYPE>
Tensor<DTYPE>* CelebADataset<DTYPE>::Normalization(Tensor<DTYPE>* image)
{
...
template <typename DTYPE>
int CelebADataset<DTYPE>::GetLength()
{
        ...
        for (int co = 0; co < width; co++)
        {
            for (int ch = 0; ch < colorDim; ch++)
            {
                ...
                imgBuf[ro * width * colorDim + co * colorDim + ch] =
                    ((*temp)[Index5D(temp->GetShape(), 0, 0, 0, 0,
                ...
                // imgBuf[ro * width * colorDim + co * colorDim + ch] =
                // ((*temp)[Index5D(temp->GetShape(), 0, 0, 0, 0, ro * width + co)]) * 255.0;
            }
        }
    }
