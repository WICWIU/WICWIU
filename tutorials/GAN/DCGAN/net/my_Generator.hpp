...
public:
    ...
    int Alloc(Operator<float>* z)
    {
        ...
        out = new TransposedConvolutionLayer2D<float>(out, 512, 256, 3, 3, 2, 2, 1, FALSE,
                                                      "G_TranspoedConv1");
        out = new BatchNormalizeLayer<float>(out, TRUE, "G_BN1");
        out = new Relu<float>(out, "G_Relu1");

        // input 7x7 output 14x14
        out = new TransposedConvolutionLayer2D<float>(out, 256, 128, 4, 4, 2, 2, 1, FALSE,
                                                      "G_TranspoedConv2");
        out = new BatchNormalizeLayer<float>(out, TRUE, "G_BN2");
        out = new Relu<float>(out, "G_Relu2");

        // input 14x14 output 28x28
        out = new TransposedConvolutionLayer2D<float>(out, 128, 1, 4, 4, 2, 2, 1, FALSE,
                                                      "G_TransposedConv3");
