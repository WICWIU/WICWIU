...
class DenseNetBlock : public Module<DTYPE>
{
private:
public:
    DenseNetBlock(Operator<DTYPE>* pInput, int pNumInputChannel, int pGrowthRate,
    ...
    int Alloc(Operator<DTYPE>* pInput, int pNumInputChannel, int pGrowthRate, std::string pName)
    {
        ...
        out = new ConvolutionLayer2D<DTYPE>(out, pNumInputChannel, 4 * pGrowthRate, 1, 1, 1, 1, 0,
                                            FALSE, "DenseNetBlock_Conv1" + pName);

        // 2
        out = new BatchNormalizeLayer<DTYPE>(out, TRUE, "DenseNetBlock_BN2" + pName);
        out = new Relu<DTYPE>(out, "DenseNetBlock_Relu2" + pName);
        out = new ConvolutionLayer2D<DTYPE>(out, 4 * pGrowthRate, pGrowthRate, 3, 3, 1, 1, 1, FALSE,
                                            "DenseNetBlock_Conv2" + pName);
...
template <typename DTYPE>
class Transition : public Module<DTYPE>
{
private:
public:
    Transition(Operator<DTYPE>* pInput, int pNumInputChannel, int pNumOutputChannel,
               std::string pName = NULL)
        : Module<DTYPE>(pName)
    {
              ...
              std::string pName)
    {
        ...
        out = new ConvolutionLayer2D<DTYPE>(out, pNumInputChannel, pNumOutputChannel, 1, 1, 1, 1, 0,
                                            FALSE, "Transition_Conv" + pName);
...
template <typename DTYPE>
class DenseNet : public NeuralNetwork<DTYPE>
{
private:
...
public:
    DenseNet(Tensorholder<DTYPE>* pInput, Tensorholder<DTYPE>* pLabel, std::string pBlockType,
             int pNumOfBlock1, int pNumOfBlock2, int pNumOfBlock3, int pNumOfBlock4,
             int pGrowthRate = 12, float pReduction = 0.5, int pNumOfClass = 1000)
    {
        Alloc(pInput, pLabel, pBlockType, pNumOfBlock1, pNumOfBlock2, pNumOfBlock3, pNumOfBlock4,
              pGrowthRate, pReduction, pNumOfClass);
    }
    ...
    int Alloc(Tensorholder<DTYPE>* pInput, Tensorholder<DTYPE>* pLabel, std::string pBlockType,
              int pNumOfBlock1, int pNumOfBlock2, int pNumOfBlock3, int pNumOfBlock4,
              int pGrowthRate, float pReduction, int pNumOfClass)
    {
        ...
        // ======================= Select Optimizer ===================
        // this->SetOptimizer(new GradientDescentOptimizer<float>(this->GetParameter(), 0.000001,
        // 0.9, 5e-4, MINIMIZE)); this->SetOptimizer(new
...
template <typename DTYPE>
NeuralNetwork<DTYPE>* DenseNet121(Tensorholder<DTYPE>* pInput, Tensorholder<DTYPE>* pLabel,
                                  int pNumOfClass)
{
    return new DenseNet<DTYPE>(pInput, pLabel, "DenseNetBlock", 6, 12, 24, 16, 12, 0.5,
                               pNumOfClass);
}

template <typename DTYPE>
NeuralNetwork<DTYPE>* DenseNetLite(Tensorholder<DTYPE>* pInput, Tensorholder<DTYPE>* pLabel,
                                   int pNumOfClass)
{
