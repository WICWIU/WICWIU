...
public:
    ...
    int Alloc(Operator<DTYPE> *pInput, int pNumInputChannel, int pNumOutputChannel, int pStride, std::string pName) {
        ...
        out = new BatchNormalizeLayer<DTYPE>(out, TRUE, TRUE, "BasicBlock_BN1" + pName);
        ...
        out = new BatchNormalizeLayer<DTYPE>(out, TRUE, TRUE, "BasicBlock_BN2" + pName);
        ...
        if ((pStride != 1) || (pNumInputChannel != pNumOutputChannel)) {
            ...
            out      = new BatchNormalizeLayer<DTYPE>(out, TRUE, TRUE, "BasicBlock_BN3" + pName);
        }
...
public:
    ...
    int Alloc(Tensorholder<DTYPE> *pInput, Tensorholder<DTYPE> *pLabel, std::string pBlockType, int pNumOfBlock1, int pNumOfBlock2, int pNumOfBlock3, int pNumOfBlock4, int pNumOfClass) {
        ...
        out = new BatchNormalizeLayer<DTYPE>(out, TRUE, TRUE, "BN0");

        // 1
        out = new ConvolutionLayer2D<DTYPE>(out, 3, m_numInputChannel, 7, 7, 2, 2, 3, TRUE, "Conv");
        out = new BatchNormalizeLayer<DTYPE>(out, TRUE, TRUE, "BN1");
