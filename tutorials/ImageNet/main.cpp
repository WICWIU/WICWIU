...
int main(int argc, char const* argv[])
{
    ...
    if (epoch / LEARNING_RATE_DECAY_TIMING)
    {
        ...
        net->GetOptimizer()->SetLearningRate(lr *
                                             pow(0.1, (int)(epoch / LEARNING_RATE_DECAY_TIMING)));
        std::cout << "lr : "
                  << lr * pow(LEARNING_RATE_DECAY_RATE, (int)(epoch / LEARNING_RATE_DECAY_TIMING))
                  << '\n';
    }
    ...
    for (int i = epoch + 1; i < EPOCH; i++)
    {
        ...
        cout << curr_tm->tm_hour << "\'h " << curr_tm->tm_min << "\'m " << curr_tm->tm_sec << "\'s"
             << endl;
        ...
        for (int j = 0; j < LOOP_FOR_TRAIN; j++)
        {
            ...
            printf("\r%d / %d -> cur_loss : %0.4f, avg_loss : %0.4f, cur_acc : %0.5f, avg_acc : "
                   "%0.5f, cur_top5_acc : %0.5f, avg_top5_acc : %0.5f" /*(ExcuteTime : %f)*/,
                   j + 1, LOOP_FOR_TRAIN, train_cur_loss, train_avg_loss / (j + 1),
                   train_cur_accuracy, train_avg_accuracy / (j + 1), train_cur_top5_accuracy,
        ...
        for (int j = 0; j < (int)LOOP_FOR_TEST; j++)
        {
            ...
            printf("\r%d / %d -> avg_loss : %0.4f, avg_acc : %0.4f, avg_top5_acc : %0.4f" /*(ExcuteTime
                                                                                             : %f)*/
                   ,
