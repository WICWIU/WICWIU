...
int main(int argc, char const* argv[])
{
    ...
    Tensorholder<float>* x_holder =
        new Tensorholder<float>(Text_length, BATCH, 1, 1, vocab_length, "x");
    Tensorholder<float>* label_holder =
        new Tensorholder<float>(Text_length, BATCH, 1, 1, vocab_length, "label");
    ...
    for (int i = epoch + 1; i < EPOCH; i++)
    {
        ...
        for (int j = 0; j < MAX_TRAIN_ITERATION; j++)
        {
            ...
            printf("\rTrain complete percentage is %d / %d -> loss : %f, acc : %f", j + 1,
                   MAX_TRAIN_ITERATION, train_avg_loss, train_accuracy);
            fflush(stdout);
        }
        ...
        for (int j = 0; j < (int)MAX_TEST_ITERATION; j++)
        {
            ...
            printf("\rTest complete percentage is %d / %d -> loss : %f, acc : %f", j + 1,
                   MAX_TEST_ITERATION, test_avg_loss / (j + 1), test_accuracy / (j + 1));
            fflush(stdout);
        }
