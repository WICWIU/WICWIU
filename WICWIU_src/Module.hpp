 ...
 * @details Operator들을 뉴럴 네트워크의 서브 그래프로 구성해 단일 Operator로서 할 수 없는 기능들을
 * 수행하게 한다
 * @details Module은 하나의 Operator처럼 뉴럴 네트워크 안에서 작동한다
 */
template <typename DTYPE>
class Module : public Operator<DTYPE>
{
private:
    /////< 신경망의 최초 Input이 되는 Operator들의 포인터를 담고 있는 Container의 포인터 멤버 변수
    Container<Operator<DTYPE>*>* m_apInput;
    /////< 신경망의 학습이 가능한 파라미터에 해당하는 Operator들의 포인터를 담고 있는 Container의
    ///포인터 멤버 변수
    Container<Operator<DTYPE>*>* m_apParameter;
    ///< Module을 구성하는 Operator들 중, 연산에 참여하는 Operator들의 포인터를 저장하는 Container
    ///< 멤버 변수
 ...
 * @details 동적으로 할당 받은 Module 클래스의 Excutable Operator Container의 메모리를 할당
 * 해제한다.
 * @return 없음
 */
template <typename DTYPE>
int Module<DTYPE>::Alloc()
{
 ...
 * @details 각 포인터들을 NULL 값으로, 각 정수 타입 변수들은 0으로 초기화하고 Module<DTYPE>::Alloc()
 * 메소드를 호출한다.
 * @see Module<DTYPE>::Alloc()
 * @return 없음
 */
template <typename DTYPE>
Module<DTYPE>::Module(std::string pName) : Operator<DTYPE>(pName)
{
#ifdef __DEBUG__
...
template <typename DTYPE>
Operator<DTYPE>* Module<DTYPE>::SetInput(Operator<DTYPE>* pInput)
{
...
template <typename DTYPE>
Operator<DTYPE>* Module<DTYPE>::SetParameter(Operator<DTYPE>* pParameter)
{
...
template <typename DTYPE>
Operator<DTYPE>* Module<DTYPE>::SetExecutableOperater(Operator<DTYPE>* pExecutableOperater)
{
...
template <typename DTYPE>
int Module<DTYPE>::SetInput(int pNumOfInput, ...)
{
 ...
 */
template <typename DTYPE>
int Module<DTYPE>::IsInput(Operator<DTYPE>* pOperator)
{
 ...
 * @details 해당 Operator의 Output container 멤버 변수에 담겨 있는 Operator들이 Module의 Excutable
 * Operator container에 중복되어 포함되어 있는 지 여부를 확인한다.
 * @param pOperator Output Container 멤버 변수가 Excutable Operator Container에 포함되어 있는 지
 * 확인하고자 하는 Operator
 * @return 해당 Operator의 Output Container 멤버 변수가 Excutable Operator Container에 중복되어
 * 포함되어 있으면 TRUE를 아니면 FALSE를 반환한다.
 */
template <typename DTYPE>
int Module<DTYPE>::IsValid(Operator<DTYPE>* pOperator)
{
 ...
 * @details 모듈의 Output에 해당하는 Operator를 매개변수로 받아 너비 우선 탐색으로 모듈 그래프를
 * 구성한다.
 * @details 매개변수로 받은 모듈의 Output에 해당하는 Operator를 시작으로 모듈의 Input에 해당하는
 * Operator까지 역순으로, Operator가 Input Tensor 및 학습 파라미터인 경우 Module 클래스의 Input
 * Container 멤버 변수에 추가하고 나머지 경우에는 Module 클래스의 Excutable Operator Container 멤버
 * 변수에 추가한다.
 * @details NeuralNetwork 클래스의 Excutable Operator Container 멤버 변수에 Operator들이 모두
 * 추가되면 Container를 역순으로 변경한다.
 * @details Operator 탐색 순서는 너비 우선 탐색을 따르며, 매개변수로 받은 Output Operator부터 해당
 * Operator의 Input Operator 리스트를 너비 우선 탐색 방식을 이용해 순서대로 진행한다.
 * @details 각 Operator들은 Module<DTYPE>::IsValid(Operator<DTYPE> *pOperator) 메소드를 이용하여
 * 모듈 그래프 안에서의 중복 여부를 확인하며 중복되는 경우 그래프에 추가하지 않는다.
 * @param pResultOperator 그래프를 구성하고자 하는 신경망의 Output에 해당하는 Operator
 * @return 매개변수로 받은 그래프를 구성하고자 하는 신경망의 Output에 해당하는 Operator
 */
template <typename DTYPE>
Operator<DTYPE>* Module<DTYPE>::AnalyzeGraph(Operator<DTYPE>* pResultOperator)
{
    // BFS
 ...
 * @details 매개변수로 받은 Tensor들을 순서대로 NeuralNetwork의 Input Container에 담겨 있는
 * Operator들의 Result로 설정한다.
 ...
 */
template <typename DTYPE>
int Module<DTYPE>::FeedInputTensor(int pNumOfInput, ...)
{
...
template <typename DTYPE>
Container<Operator<DTYPE>*>* Module<DTYPE>::GetExcutableOperatorContainer()
{
    return m_aaExcutableOperator;
}

template <typename DTYPE>
int Module<DTYPE>::GetNumOfExcutableOperator()
{
    return m_numOfExcutableOperator;
}

template <typename DTYPE>
Tensor<DTYPE>* Module<DTYPE>::GetResult() const
{
    return m_pLastOperator->GetResult();
}

template <typename DTYPE>
Container<Tensor<DTYPE>*>* Module<DTYPE>::GetResultContainer()
{
    return m_pLastOperator->GetResultContainer();
}

template <typename DTYPE>
Tensor<DTYPE>* Module<DTYPE>::GetGradient() const
{
    return m_pLastOperator->GetGradient();
}

template <typename DTYPE>
Container<Tensor<DTYPE>*>* Module<DTYPE>::GetGradientContainer()
{
    return m_pLastOperator->GetGradientContainer();
}

template <typename DTYPE>
Tensor<DTYPE>* Module<DTYPE>::GetDelta() const
{
    return m_pLastOperator->GetDelta();
}

template <typename DTYPE>
Container<Tensor<DTYPE>*>* Module<DTYPE>::GetDeltaContainer()
{
    return m_pLastOperator->GetDeltaContainer();
}

template <typename DTYPE>
Container<Operator<DTYPE>*>* Module<DTYPE>::GetParameterContainer()
{
...
template <typename DTYPE>
Container<Operator<DTYPE>*>* Module<DTYPE>::GetParameter()
{
    return m_apParameter;
    // return this->GetInputContainer();
}
...
template <typename DTYPE>
int Module<DTYPE>::SetIsTensorholder(int pIsParameter)
{
...
template <typename DTYPE>
int Module<DTYPE>::SetIsTrainable(int pIsTrainable)
{
    for (int i = 0; i < m_ParameterDegree; i++)
    {
...
template <typename DTYPE>
int Module<DTYPE>::SetModeAccumulate()
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
...
template <typename DTYPE>
int Module<DTYPE>::SetModeInference()
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
 ...
 * @details Excutable Operator Container의 각 Operator들에서 Operator<DTYPE>::ForwardPropagate(int
 * pTime) 메소드를 순서대로 호출한다.
 * @param pTime 각 ForwardPropagate 메소드에 전달할 Time의 인덱스
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::ForwardPropagate(int pTime)
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
 ...
 * @details 역순으로 Excutable Operator Container의 각 Operator들에서
 * Operator<DTYPE>::ForwardPropagate(int pTime) 메소드를 호출한다.
 * @param pTime 각 ForwardPropagate 메소드에 전달할 Time의 인덱스
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::BackPropagate(int pTime)
{
 ...
 * @details Excutable Operator Container에 포함되어 있는 각 Operator들에서
 * Operator<DTYPE>::ResetResult() 메소드를 호출한다.
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::ResetResult()
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
 ...
 * @details Excutable Operator Container에 포함되어 있는 각 Operator들에서
 * Operator<DTYPE>::ResetGradient() 메소드를 호출한다.
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::ResetGradient()
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
...
template <typename DTYPE>
void Module<DTYPE>::PrintInformation(int level)
{
 ...
 * @details Module의 Device 멤버변수를 CPU로 전환하고, Excutable Operator Container의 각
 * Operator들에서 Operator<DTYPE>::SetDeviceCPU() 메소드를 순서대로 호출한다.
 * @return 없음
 */
template <typename DTYPE>
void Module<DTYPE>::SetDeviceCPU()
{
    if (m_Device != CPU)
...
template <typename DTYPE>
void Module<DTYPE>::SetDeviceCPUOnModule()
{
    this->SetDevice(CPU);
...
template <typename DTYPE>
int Module<DTYPE>::Save(char* nameOfFile)
{
...
template <typename DTYPE>
int Module<DTYPE>::Load(char* nameOfFile)
{
...
template <typename DTYPE>
int Module<DTYPE>::SaveComponents(char* nameOfDir)
{
    char filename[256];
...
template <typename DTYPE>
int Module<DTYPE>::LoadComponents(char* nameOfDir)
{
    char filename[256];
 ...
 */
template <typename DTYPE>
void Module<DTYPE>::SetDeviceGPU(cudnnHandle_t& pCudnnHandle, unsigned int idOfDevice)
{
    if (this->GetDevice() != GPU)
        this->SetDeviceGPUOnModule(pCudnnHandle, idOfDevice);
}

template <typename DTYPE>
void Module<DTYPE>::SetDeviceGPUOnModule(cudnnHandle_t& pCudnnHandle, unsigned int idOfDevice)
{
 ...
 * @details Excutable Operator Container의 각 Operator들에서
 * Operator<DTYPE>::ForwardPropagateOnGPU(int pTime) 메소드를 순서대로 호출한다.
 * @param pTime 각 ForwardPropagateOnGPU 메소드에 전달할 Time의 인덱스
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::ForwardPropagateOnGPU(int pTime)
{
    for (int i = 0; i < m_numOfExcutableOperator; i++)
    {
 ...
 * @details 역순으로 Excutable Operator Container의 각 Operator들에서
 * Operator<DTYPE>::BackPropagateOnGPU(int pTime) 메소드를 호출한다.
 * @param pTime 각 BackPropagateOnGPU 메소드에 전달할 Time의 인덱스
 * @return TRUE
 */
template <typename DTYPE>
int Module<DTYPE>::BackPropagateOnGPU(int pTime)
{
    for (int i = m_numOfExcutableOperator - 1; i >= 0; i--)
    {
