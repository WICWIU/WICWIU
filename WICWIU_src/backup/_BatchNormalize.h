...
public:
    ...
    BatchNormalize(Operator<DTYPE>* pInput, Operator<DTYPE>* pScale, Operator<DTYPE>* pBias,
                   int pIsChannelwise, std::string pName)
        : Operator<DTYPE>(pName)
    {
#ifdef __DEBUG__
        std::cout << "BatchNormalize:: BatchNormalize( Operator< DTYPE>*, Operator< DTYPE>*, "
                     "Operator< DTYPE>*, int, std:: string)"
                  << '\n';
#endif // __DEBUG__
...
#ifdef __DEBUG__
        std::cout << "BatchNormalize:: BatchNormalize( Operator< DTYPE>*, Operator< DTYPE>*, "
                     "Operator< DTYPE>*, int, float, std:: string)"
                  << '\n';
#endif // __DEBUG__
        ...
        for (int example = 0; example < m_inputBatchSize; example++)
        {
            for (int channel = 0; channel < m_numChannel; channel++)
            {
                for (int inputRow = 0; inputRow < m_numInputRow; inputRow++)
                {
                    ...
                    {
                        inputIndex =
                            Index4D(m_pInputShape, example, channel, inputRow, inputColumn);
                        batchSummaryIndex = GetBatchSummaryIndex(channel, inputRow, inputColumn);

                        derResultValue = (*m_pTenDerResult)[inputIndex];

                        (*m_pTenDerScale)[batchSummaryIndex] +=
                            derResultValue * (*m_aTenNormalizedInput)[inputIndex];
            ...
            for (int channel = 0; channel < m_numChannel; channel++)
            {
                for (int inputRow = 0; inputRow < m_numInputRow; inputRow++)
                {
                    for (int inputColumn = 0; inputColumn < m_numInputColumn; inputColumn++)
                    {
                        ...
                        (*m_pTenDerInput)[inputIndex] =
                            ((*m_pTenDerResult)[inputIndex] -
                             ((*m_pTenDerBias)[batchSummaryIndex] +
                              (*m_pTenDerScale)[batchSummaryIndex] *
                                  (*m_aTenNormalizedInput)[inputIndex]) /
                                 m_effectiveBatchSize) *
                            (*m_pTenScale)[batchSummaryIndex] /
                            (*m_aTenBatchStandardDeviation)[batchSummaryIndex];
                    }
                }
            }
        }
...
private:
    ...
    void Allocate(Operator<DTYPE>* pInput, Operator<DTYPE>* pScale, Operator<DTYPE>* pBias,
                  int pIsChannelwise, float pEpsilon)
    {
        Operator<DTYPE>::Alloc(3, pInput, pScale, pBias);

        std::cout << "BatchNormalize:: Allocate( Operator< DTYPE>*, Operator< DTYPE>*, Operator< "
                     "DTYPE>*, int, float)"
                  << '\n';
        ...
        m_aBatchSummaryShape =
            new Shape(1, 1, m_numChannel, m_numBatchSummaryRow, m_numBatchSummaryColumn);
    ...
    void ComputeBatchSummary()
    {
            ...
            for (int channel = 0; channel < m_numChannel; channel++)
            {
                for (int inputRow = 0; inputRow < m_numInputRow; inputRow++)
                {
                    for (int inputColumn = 0; inputColumn < m_numInputColumn; inputColumn++)
                    {
                        ...
                        inputValue = (*m_pTenInput)[Index4D(m_pInputShape, example, channel,
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            ...
            {
                for (int batchSummaryColumn = 0; batchSummaryColumn < m_numBatchSummaryColumn;
                     batchSummaryColumn++)
                {
                    batchSummaryIndex = Index4D(m_aBatchSummaryShape, 0, channel, batchSummaryRow,
                                                batchSummaryColumn);

                    meanValue = (*m_aTenBatchMean)[batchSummaryIndex] / m_effectiveBatchSize;
                    standardDeviationValue =
                        SqrtStable(((*m_aTenBatchStandardDeviation)[batchSummaryIndex] /
                                    m_effectiveBatchSize) -
                                   (meanValue * meanValue));
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            for (int batchSummaryRow = 0; batchSummaryRow < m_numBatchSummaryRow; batchSummaryRow++)
            {
                     ...
                     batchSummaryColumn++)
                {
                    ...
                    (*m_aTenTotalStandardDeviation)[batchSummaryIndex] +=
            ...
            for (int channel = 0; channel < m_numChannel; channel++)
            {
                for (int inputRow = 0; inputRow < m_numInputRow; inputRow++)
                {
                    for (int inputColumn = 0; inputColumn < m_numInputColumn; inputColumn++)
                    {
                        ...
                        deviationValue =
                            (*m_pTenInput)[inputIndex] - (*m_aTenBatchMean)[batchSummaryIndex];

                        (*m_aTenNormalizedInput)[inputIndex] =
                            deviationValue / (*m_aTenBatchStandardDeviation)[batchSummaryIndex];
                    }
                }
            }
        }
            ...
            for (int channel = 0; channel < m_numChannel; channel++)
            {
                for (int inputRow = 0; inputRow < m_numInputRow; inputRow++)
                {
                    for (int inputColumn = 0; inputColumn < m_numInputColumn; inputColumn++)
                    {
                        ...
                        (*m_pTenResult)[inputIndex] =
                            ((*m_pTenScale)[batchSummaryIndex] * (*pTenInput)[inputIndex]) +
                            (*m_pTenBias)[batchSummaryIndex];
                    }
                }
            }
        }
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            for (int batchSummaryRow = 0; batchSummaryRow < m_numBatchSummaryRow; batchSummaryRow++)
            {
                     ...
                     batchSummaryColumn++)
                {
                    ...
                    varianceValue =
                        ((*m_aTenTotalStandardDeviation)[batchSummaryIndex] / m_numBatch) /
                        varianceBias;
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            for (int batchSummaryRow = 0; batchSummaryRow < m_numBatchSummaryRow; batchSummaryRow++)
            {
                     ...
                     batchSummaryColumn++)
                {
                    ...
                    (*m_pTenBias)[batchSummaryIndex] -=
                        scaleValue * (*m_aTenTotalMean)[batchSummaryIndex];
                }
            }
        }
    }
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            for (int batchSummaryRow = 0; batchSummaryRow < m_numBatchSummaryRow; batchSummaryRow++)
            {
                     ...
                     batchSummaryColumn++)
                {
                    ...
                    (*m_pTenScale)[batchSummaryIndex] =
                        scaleValue * (*m_aTenTotalStandardDeviation)[batchSummaryIndex];
                    (*m_pTenBias)[batchSummaryIndex] +=
                        scaleValue * (*m_aTenTotalMean)[batchSummaryIndex];
                }
            }
        ...
        for (int channel = 0; channel < m_numChannel; channel++)
        {
            for (int batchSummaryRow = 0; batchSummaryRow < m_numBatchSummaryRow; batchSummaryRow++)
            {
                     ...
                     batchSummaryColumn++)
                {
                    ...
                    (*m_aTenTotalMean)[batchSummaryIndex] *= m_numBatch;
                    (*m_aTenTotalStandardDeviation)[batchSummaryIndex] =
                        standardDeviationValue * standardDeviationValue * m_numBatch * varianceBias;
                }
            }
        }
    }
    ...
    float SqrtStable(float base) { return std::sqrt(base + m_epsilon); }
};
