...
public:
    DataLoader(Dataset<DTYPE>* dataset, int batchSize = 1, int useShuffle = FALSE,
...
template <typename DTYPE>
DataLoader<DTYPE>::DataLoader(Dataset<DTYPE>* dataset, int batchSize, int useShuffle,
                              int numOfWorker, int dropLast)
{
#ifdef __DEBUG___
    std::cout << __FUNCTION__ << '\n';
    std::cout << __FILE__ << '\n';
...
template <typename DTYPE>
void DataLoader<DTYPE>::StartProcess()
{
    // Generate thread for Dist - DistributeIdxOfData2Thread()
    // Generate thread set for Process -
    m_nowWorking = TRUE;

    m_aThreadForDistInfo =
        std::thread([&]() { this->DistributeIdxOfData2Thread(); }); // lambda expression
    printf("Generate dataloader base thread\r\n");

    for (int i = 0; i < m_numOfWorker; i++)
    {
        m_aaWorkerForProcess[i] =
            std::thread([&]() { this->DataPreprocess(); }); // lambda expression
        printf("Generate worker[%d] for data preprocessing\r\n", i);
    }
}

template <typename DTYPE>
void DataLoader<DTYPE>::StopProcess()
{
    // Stop Thread
    // not deallocate!
    m_nowWorking = FALSE;
...
template <typename DTYPE>
void DataLoader<DTYPE>::MakeAllOfIndex(std::vector<int>* pAllOfIndex)
{
...
template <typename DTYPE>
void DataLoader<DTYPE>::DistributeIdxOfData2Thread()
{
    ...
    while (m_nowWorking)
    {
        ...
        if (cnt == numOfBatchBlockSize)
        {
            if (!m_dropLast && dropLastSize)
            {
                ...
                if (m_useShuffle)
                    std::random_shuffle(allOfIdx->begin() + dropLastSize, allOfIdx->end(),
                                        random_generator);
            }
            else
            {
...
template <typename DTYPE>
void DataLoader<DTYPE>::DataPreprocess()
{
    // for thread
...
template <typename DTYPE>
void DataLoader<DTYPE>::Push2IdxBuffer(std::vector<int>* setOfIdx)
{
...
template <typename DTYPE>
std::vector<int>* DataLoader<DTYPE>::GetIdxSetFromIdxBuffer()
{
...
template <typename DTYPE>
Tensor<DTYPE>* DataLoader<DTYPE>::Concatenate(std::queue<Tensor<DTYPE>*>& setOfData)
{
...
template <typename DTYPE>
void DataLoader<DTYPE>::Push2GlobalBuffer(std::vector<Tensor<DTYPE>*>* preprocessedData)
{
...
template <typename DTYPE>
std::vector<Tensor<DTYPE>*>* DataLoader<DTYPE>::GetDataFromGlobalBuffer()
{
