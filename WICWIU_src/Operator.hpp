...
public:
    ...
    Operator(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1, std::string pName = "NO NAME",
             int pLoadflag = TRUE);
    Operator(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1, Operator<DTYPE>* pInput2,
 ...
 * @details AddEdgebetweenOperators매소드를 통해 파라미터로 전달받은 Operator의 주소값을 이용해을
 * 연결시킨다.
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::Alloc(int numInput, ...)
{
#ifdef __DEBUG__
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddInputEdge(Operator<DTYPE>* pInput)
{
    try
    {
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddOutputEdge(Operator<DTYPE>* pOutput)
{
    try
    {
...
/*!
 * @brief Operator의 생성자.
 * @details 파라미터로 전달 받은 pName을 m_name으로 설정 하고 나머지는 변수들은 NULL, CPU,
 * TRAINING으로 초기화 한다.
 * @param pName 사용자가 설정 할 Operator의 이름.
 */
template <typename DTYPE>
Operator<DTYPE>::Operator(std::string pName, int pLoadflag)
{
#ifdef __DEBUG__
...
/*!
 * @brief Operator의 생성자.
 * @details파라미터로 전달 받은 pName을 m_name으로 설정 하고 나머지는 변수들은 NULL, CPU,
 * TRAINING으로 초기화 한 뒤, AddEdgebetweenOperators를 통해 Operator들을 서로 연결한다.
 * @param pInput Operator와 연결 할 Operator들의 주소 값들.
 * @param pName 사용자가 설정 할 Operator의 이름.
 * @ref Operator<DTYPE>::AddEdgebetweenOperators(int numInput, ...)
 */
template <typename DTYPE>
Operator<DTYPE>::Operator(Operator<DTYPE>* pInput, std::string pName, int pLoadflag)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Operator()" << '\n';
#endif // __DEBUG__
 ...
 */
template <typename DTYPE>
Operator<DTYPE>::Operator(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1, std::string pName,
                          int pLoadflag)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Operator()" << '\n';
 ...
 */
template <typename DTYPE>
Operator<DTYPE>::Operator(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1,
                          Operator<DTYPE>* pInput2, std::string pName, int pLoadflag)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Operator()" << '\n';
#endif // __DEBUG__
...
template <typename DTYPE>
Operator<DTYPE>::Operator(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1,
                          Operator<DTYPE>* pInput2, Operator<DTYPE>* pInput3, std::string pName,
                          int pLoadflag)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Operator()" << '\n';
...
template <typename DTYPE>
Operator<DTYPE>::Operator(int numInput, ...)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Operator()" << '\n';
#endif // __DEBUG__
 ...
 * @ref int Operator<DTYPE>::AddInputEdge(Operator<DTYPE> *pInput), int
 * Operator<DTYPE>::AddOutputEdge(Operator<DTYPE> *pOutput)
 * @return 성공 시 TRUE.
 */
template <typename DTYPE>
...
template <typename DTYPE>
int Operator<DTYPE>::AddEdgebetweenOperators(int numInput, va_list ap)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Alloc(Tensor<DTYPE> *)" << '\n';
#endif // __DEBUG__
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddEdgebetweenOperators(int numInput, ...)
{
#ifdef __DEBUG__
    std::cout << "Operator<DTYPE>::Alloc(Tensor<DTYPE> *)" << '\n';
#endif // __DEBUG__
...
template <typename DTYPE>
int Operator<DTYPE>::SetResult(Tensor<DTYPE>* pTensor)
{
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddResult(Tensor<DTYPE>* pTensor)
{
    m_aaResult->Push(pTensor);
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetGradient(Tensor<DTYPE>* pTensor)
{
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddGradient(Tensor<DTYPE>* pTensor)
{
    m_aaGradient->Push(pTensor);
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetDelta(Tensor<DTYPE>* pTensor)
{
    if (m_aaGradient->GetSize())
    {
        Tensor<DTYPE>* temp = m_aaGradient->Pop();
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::AddDelta(Tensor<DTYPE>* pTensor)
{
    m_aaGradient->Push(pTensor);
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetDevice(Device pDevice)
{
    m_Device = pDevice;
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetDeviceID(unsigned int idOfDevice)
{
...
template <typename DTYPE>
int Operator<DTYPE>::SetIsTensorholder(int pIsParameter)
{
...
template <typename DTYPE>
int Operator<DTYPE>::SetIsTrainable(int pIsTrainable)
{
...
template <typename DTYPE>
int Operator<DTYPE>::SetModeTrain()
{
    m_Mode = TRAIN;
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetModeAccumulate()
{
    m_Mode = ACCUMULATE;
    return TRUE;
}
...
template <typename DTYPE>
int Operator<DTYPE>::SetModeInference()
{
    m_Mode = INFERENCE;
    return TRUE;
}
...
template <typename DTYPE>
Operator<DTYPE>** Operator<DTYPE>::GetOutput()
{
    return m_apOutput->GetRawData();
}

template <typename DTYPE>
Container<Operator<DTYPE>*>* Operator<DTYPE>::GetOutputContainer()
{
    return m_apOutput;
}

template <typename DTYPE>
Operator<DTYPE>** Operator<DTYPE>::GetInput()
{
    return m_apInput->GetRawData();
}

template <typename DTYPE>
Container<Operator<DTYPE>*>* Operator<DTYPE>::GetInputContainer()
{
    return m_apInput;
}

template <typename DTYPE>
Tensor<DTYPE>* Operator<DTYPE>::GetResult() const
{
    return (*m_aaResult)[0];
}

template <typename DTYPE>
Container<Tensor<DTYPE>*>* Operator<DTYPE>::GetResultContainer()
{
    return m_aaResult;
}

template <typename DTYPE>
Tensor<DTYPE>* Operator<DTYPE>::GetGradient() const
{
    return (*m_aaGradient)[0];
}

template <typename DTYPE>
Container<Tensor<DTYPE>*>* Operator<DTYPE>::GetGradientContainer()
{
    return m_aaGradient;
}

template <typename DTYPE>
Tensor<DTYPE>* Operator<DTYPE>::GetDelta() const
{
    return (*m_aaGradient)[0];
    // return (*m_aaDelta)[0];
}
...
template <typename DTYPE>
Container<Tensor<DTYPE>*>* Operator<DTYPE>::GetDeltaContainer()
{
    return m_aaGradient;
    // return m_aaDelta;
}
...
template <typename DTYPE>
std::string Operator<DTYPE>::GetName() const
{
    return m_name;
}

template <typename DTYPE>
Device Operator<DTYPE>::GetDevice()
{
    return m_Device;
}

template <typename DTYPE>
int Operator<DTYPE>::GetDeviceID()
{
    return m_idOfDevice;
}

template <typename DTYPE>
int Operator<DTYPE>::GetIsTensorholder()
{
    return m_isParameter;
}

template <typename DTYPE>
int Operator<DTYPE>::GetIsTrainable()
{
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::ForwardPropagate(int pTime)
{
#ifdef __DEBUG__
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::BackPropagate(int pTime)
{
#ifdef __DEBUG__
...
template <typename DTYPE>
int Operator<DTYPE>::ResetResult()
{
...
template <typename DTYPE>
int Operator<DTYPE>::ResetGradient()
{
 ...
 */
template <typename DTYPE>
void Operator<DTYPE>::PrintInformation(int level)
{
...
template <typename DTYPE>
void Operator<DTYPE>::SetDeviceCPU()
{
    if (m_Device != CPU)
    {
        this->SetDevice(CPU);
...
template <typename DTYPE>
int Operator<DTYPE>::SetResultOnCPU()
{
    // Tensorholder의 경우는 하면 안된다.
    int size = m_aaResult->GetSize();
...
template <typename DTYPE>
int Operator<DTYPE>::SetGradientOnCPU()
{
    int size = m_aaGradient->GetSize();
...
template <typename DTYPE>
int Operator<DTYPE>::Save(char* nameOfFile)
{
...
template <typename DTYPE>
int Operator<DTYPE>::Load(char* nameOfFile)
{
...
template <typename DTYPE>
int Operator<DTYPE>::Save(FILE* fp)
{
    int size = m_aaResult->GetSize();
...
template <typename DTYPE>
int Operator<DTYPE>::Load(FILE* fp)
{
    int size = m_aaResult->GetSize();
...
template <typename DTYPE>
int Operator<DTYPE>::SetCudnnHandle(cudnnHandle_t& pCudnnHandle)
{
...
template <typename DTYPE>
int Operator<DTYPE>::SetResultOnGPU(unsigned int idOfDevice)
{
    // Tensorholder의 경우는 하면 안된다.
    int size = m_aaResult->GetSize();
...
template <typename DTYPE>
int Operator<DTYPE>::SetGradientOnGPU(unsigned int idOfDevice)
{
    int size = m_aaGradient->GetSize();
...
template <typename DTYPE>
void Operator<DTYPE>::InitializeAttributeForGPU(unsigned int idOfDevice)
{
}
 ...
 */
template <typename DTYPE>
void Operator<DTYPE>::SetDeviceGPU(cudnnHandle_t& pCudnnHandle, unsigned int idOfDevice)
{
    if (m_Device != GPU)
    {
...
template <typename DTYPE>
cudnnHandle_t& Operator<DTYPE>::GetCudnnHandle()
{
 ...
 */
template <typename DTYPE>
int Operator<DTYPE>::ForwardPropagateOnGPU(int pTime)
{
#if __DEBUG__
...
template <typename DTYPE>
int Operator<DTYPE>::BackPropagateOnGPU(int pTime)
{
#if __DEBUG__
