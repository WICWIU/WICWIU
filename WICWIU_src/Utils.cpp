...
float GetSquareDistance(int dim, float* pVec1, float* pVec2)
{
    ...
    for (int i = 0; i < dim; i++)
    {
        ...
        {
            printf("abnormal: diff = %f, pVec1[%d] = %f, pVec2[%d] = %f", diff, i, pVec1[i], i,
                   pVec2[i]);
        ...
        {
            printf("abnormal: dist = %f diff = %f, pVec1[%d] = %f, pVec2[%d] = %f", dist, diff, i,
                   pVec1[i], i, pVec2[i]);
            printf("Press Enter to continue... in %s", __FUNCTION__);
            fflush(stdout);
            getchar();
