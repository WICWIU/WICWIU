...
/*!
@class Linear Operator들을 그래프로 구성해 fully connected layer의 기능을 수행하는 모듈을 생성하는
클래스
...
public:
    ...
    @see linear<DTYPE>::Alloc(Operator<DTYPE> *pInput, int pNumInputCol, int pNumOutputCol, int
    use_bias, std::string pName)
    */
    Linear(Operator<DTYPE>* pInput, int pNumInputCol, int pNumOutputCol, int use_bias = FALSE,
    ...
    */
    ...
    @details Input Operator의 Element에 대해 Weight를 이용해 행렬 곱(Matrix Multiplication)을
    ...
    @param pNumOutputCol 해당 Layer의 Output Operator의 Column의 갯수, Output Column에 대한
    Dimension
    @param use_bias Bias 사용 유무, 0일 시 사용 안 함, 0이 아닐 시 사용
    @param pName Module의 이름
    @return TRUE
    @see MatMul<DTYPE>::MatMul(Operator<DTYPE> *pWeight, Operator<DTYPE> *pInput, std::string pName)
    ...
    */
    int Alloc(Operator<DTYPE>* pInput, int pNumInputCol, int pNumOutputCol, int use_bias,
              std::string pName)
    {
        ...
        Tensorholder<DTYPE>* pWeight = new Tensorholder<DTYPE>(
            Tensor<DTYPE>::Random_normal(1, 1, 1, pNumOutputCol, pNumInputCol, 0.0, stddev),
            "Layer_Weight_" + pName);
        ...
        if (use_bias)
        {
            Tensorholder<DTYPE>* pBias = new Tensorholder<DTYPE>(
                Tensor<DTYPE>::Constants(1, 1, 1, 1, pNumOutputCol, 0.f), "Add_Bias_" + pName);
