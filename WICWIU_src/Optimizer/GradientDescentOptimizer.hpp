...
public:
    ...
    @see int Alloc()
    */
    GradientDescentOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                             OptimizeDirection pOptimizeDirection)
        : Optimizer<DTYPE>(pParameterContainer, pLearningRate, pOptimizeDirection)
    {
#ifdef __DEBUG__
        std::cout << "GradientDescentOptimizer::GradientDescentOptimizer(LossFunction<DTYPE> *, "
                     "float, OptimizeDirection)"
                  << '\n';
#endif // __DEBUG__
        m_ppParameter = NULL;
        m_aaVelocity = NULL;
    ...
    */
    GradientDescentOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                             float momentum, OptimizeDirection pOptimizeDirection)
        : Optimizer<DTYPE>(pParameterContainer, pLearningRate, pOptimizeDirection)
    {
#ifdef __DEBUG__
        std::cout << "GradientDescentOptimizer::GradientDescentOptimizer(LossFunction<DTYPE> *, "
    ...
    GradientDescentOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                             float momentum, float clipValue, OptimizeDirection pOptimizeDirection)
        : Optimizer<DTYPE>(pParameterContainer, pLearningRate, pOptimizeDirection)
    {
#ifdef __DEBUG__
        std::cout << "GradientDescentOptimizer::GradientDescentOptimizer(LossFunction<DTYPE> *, "
    ...
    */
    GradientDescentOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                             float momentum, float clipValue, float weightDecayRate,
                             OptimizeDirection pOptimizeDirection)
    ...
    */
    ...
    /*!
    @brief 파라미터 값들을 업데이트 하는 메소드
    @details Parameter안에 있는 Tensor의 새로 계산된 gradinet값과 learning_rate의 곱,
    weightDecayRate와 기존 weight(trainable_date)의 곱으로 weight(trainable_date)값을 업데이트한다.
    ...
    */
    int UpdateParameter(Operator<DTYPE>* pParameter)
    {
        ...
        {
            (*trainable_data)[i] +=
                (learning_rate * (*gradient)[i] + weightDecayRate * (*trainable_data)[i]);
        }
    ...
    @details Parameter안에 있는 Tensor의 새로 계산된 gradinet값과 learning_rate의 곱,
    weightDecayRate와 기존 weight(trainable_date)의 곱으로 weight(trainable_date)값을 업데이트한다.
    @details momentum과 pVelocity의 곱과 learnung_rate와 gradient의 곱으로 pVelocity의 값을 업데이트
    한다.
    @param pParameter 업데이트 할 Tensor를 가지고 있는 Operator포인터
    ...
    */
    int UpdateParameterOnGPU(Operator<DTYPE>* pParameter)
    {
        ...
        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &beta, gradDesc, m_pDevGrad, &alpha,
                                  dataDesc, m_pDevData));
    ...
    */
    int UpdateParameterOnGPU(Operator<DTYPE>* pParameter, Tensor<DTYPE>* pVelocity)
    {
        ...
        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &beta, gradDesc, m_pDevGrad, &m_momentum,
                                  veloDesc, m_pDevVelo));

        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &alpha, veloDesc, m_pDevVelo, &alpha,
                                  dataDesc, m_pDevData));
