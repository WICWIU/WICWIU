...
@see  int NagOptimizer<DTYPE>::UpdateParameterOnGPU(Operator<DTYPE> *pParameter, Tensor<DTYPE>
*pVelocity)
*/
__global__ void NagUpdate_kernel(float* pDevWeight, float* pDevAccGradient, int weightDim,
                                 float signed_learning_rate, float momentum, float weightDecayRate,
                                 float* pDevVelocity)
{
...
@see __global__ void NagUpdate_kernel(float *pDevWeight, float *pDevAccGradient, int weightDim,
float signed_learning_rate, float momentum, float weightDecayRate, float *pDevVelocity)
@param *pParameter 업데이트 할 Tensor를 가지고 있는 Operator포인터
@param pVelocity 업데이트할 pVelocity
@return 성공 시 TRUE
*/
template <typename DTYPE>
int NagOptimizer<DTYPE>::UpdateParameterOnGPU(Operator<DTYPE>* pParameter, Tensor<DTYPE>* pVelocity)
{
    ...
    NagUpdate_kernel<<<noBlock, threadsPerBlock>>>(m_pDevData, m_pDevGrad, m_parameterDim,
                                                   signed_learning_rate, m_momentum,
                                                   weightDecayRate, m_pDevVelocity);
