...
@see int AdamOptimizer<DTYPE>::UpdateParameterOnGPU(Operator<DTYPE> *pParameter, Tensor<DTYPE>
*pFirstMomentum, Tensor<DTYPE> *pFirstVelocity, Tensor<DTYPE> *pUnbiasedMomentum, Tensor<DTYPE>
*pUnbiasedVelocity)
*/
__global__ void AdamUpdate_kernel(float* pDevWeight, float* pDevAccGradient, int weightDim,
                                  float signed_learning_rate, float beta1, float beta2,
                                  float epsilon, float weightDecayRate, float* pDevFirstMomentum,
                                  float* pDevFirstVelocity)
{
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < weightDim;
         idx += blockDim.x * gridDim.x)
    {
        float g = pDevAccGradient[idx];
        pDevFirstMomentum[idx] =
            beta1 * pDevFirstMomentum[idx] + (1.F - beta1) * g; // m (1st moment)
        pDevFirstVelocity[idx] =
            beta2 * pDevFirstVelocity[idx] + (1.F - beta2) * g * g; // v (2nd moment)
...
@see __global__ void AdamUpdate_kernel(float *pDevWeight, float *pDevAccGradient, int weightDim,
float signed_learning_rate, float beta1, float beta2, float epsilon, float weightDecayRate, float
*pDevFirstMomentum, float *pDevFirstVelocity)
...
int AdamOptimizer<DTYPE>::UpdateParameterOnGPU(Operator<DTYPE>* pParameter,
                                               Tensor<DTYPE>* pFirstMomentum,
                                               Tensor<DTYPE>* pFirstVelocity,
                                               Tensor<DTYPE>* pUnbiasedMomentum,
                                               Tensor<DTYPE>* pUnbiasedVelocity)
{
    ...
    AdamUpdate_kernel<<<noBlock, threadsPerBlock>>>(
        m_pDevData, m_pDevGrad, m_parameterDim, signed_learning_rate, m_Beta1, m_Beta2, m_epsilon,
        weightDecayRate, m_pDevFirstMomentum, m_pDevFirstVelocity);
