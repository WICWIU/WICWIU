...
trainable_data = momentum * trainable_data{t-1} + learning_rate * g_t / sqrt(m_aaMeanSqueared +
epsilon)
...
trainable_data = momentum * mom{t-1} + learning_rate * g_t /  sqrt(mean_square - mean_grad**2 +
epsilon)
...
class RMSPropOptimizer : public Optimizer<DTYPE>
{
private:
    Container<Operator<DTYPE>*>* m_ppParameter;
    ///< 값을 업데이트 할 Tensor들을 가리키는 포인터
    Container<Tensor<DTYPE>*>*
        m_aaMeanSquared; //  = decay * mean_square{t-1} + (1-decay) * gradient ** 2
...
public:
    ...
    */
    RMSPropOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                     float decay, float epsilon, bool centered,
                     OptimizeDirection pOptimizeDirection)
        : Optimizer<DTYPE>(pParameterContainer, pLearningRate, pOptimizeDirection)
    {
#ifdef __DEBUG__
        std::cout << "RMSPropOptimizer::RMSPropOptimizer(LossFunction<DTYPE> *, float, float, "
                     "OptimizeDirection)"
                  << '\n';
#endif // __DEBUG__
        m_ppParameter = NULL;
    ...
    */
    RMSPropOptimizer(Container<Operator<DTYPE>*>* pParameterContainer, float pLearningRate,
                     float decay, float epsilon, bool centered, float weightDecayRate,
                     OptimizeDirection pOptimizeDirection)
    ...
    */
    int UpdateParameter(Operator<DTYPE>* pParameter, Tensor<DTYPE>* m_pMeanSquared)
    {
        ...
        float signed_learning_rate =
            this->GetOptimizeDirection() * this->GetLearningRate(); // minimizer = -1
        int capacity = trainable_data->GetCapacity();

        for (int i = 0; i < capacity; i++)
        {
            (*m_pMeanSquared)[i] = (m_decay * (*m_pMeanSquared)[i]) +
                                   ((1.f - m_decay) * ((*gradient)[i] * (*gradient)[i]));
            (*trainable_data)[i] += ((signed_learning_rate * (*gradient)[i]) /
                                     std::sqrt((*m_pMeanSquared)[i] + m_epsilon));
        }
    ...
    @details m_decay 가중치로 조정된 m_pMeanSquared, gradinet로 m_pMeanSquared 업데이트 한다.
    @details 업데이트 된 m_pMeanSquared와 업데이트 된 m_pMeanGrad의 제곱의 차를 지수평균이동공식에
    적용하여 파라미터를 업데이트 한다.
    @param pParameter 업데이트 할 Tensor를 가지고 있는 Operator포인터
    @param pMeanSquared 업데이트 할 pMeanSquared
        ...
        for (int i = 0; i < capacity; i++)
        {
            ...
            (*trainable_data)[i] +=
                ((signed_learning_rate * (*gradient)[i]) /
                 std::sqrt(((*m_pMeanSquared)[i] - ((*m_pMeanGrad)[i] * (*m_pMeanGrad)[i])) +
                           m_epsilon));
        }
