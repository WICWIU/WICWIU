 ...
 * @details Operator, Module, Loss Function, Optimizer 클래스를 생성 및 활용해 뉴럴 네트워크를
 * 구성하고 학습시킨다
 */
template <typename DTYPE>
class NeuralNetwork : public Module<DTYPE>
{
private:
...
public:
    ...
    void InputToFeature(int inDim, int noSample, float* pSamples[], int outDim, float* pFeatures[],
                        int batchSize = 32);
 ...
 * @details NeuralNetwork 클래스의 Operator, Excutable Operator, Input, Parameter Container들 각각에
 * 대해 메모리를 동적으로 할당한다.
 * @return TRUE
 */
template <typename DTYPE>
 ...
 * @details 동적으로 할당 받은 NeuralNetwork 클래스의 Operator, Excutable Operator, Input, Parameter
 * Container들과 LossFunction, Optimizer의 메모리를 할당 해제한다.
 * @return 없음
 */
template <typename DTYPE>
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::AllocOnGPU()
{
 ...
 */
template <typename DTYPE>
void NeuralNetwork<DTYPE>::DeleteOnGPU()
{
 ...
 * @details 각 포인터들을 NULL 값으로, 각 정수 타입 변수들은 0으로, Device는 CPU로 초기화하고
 * NeuralNetwork<DTYPE>::Alloc() 메소드를 호출한다.
 * @return 없음
 * @see NeuralNetwork<DTYPE>::Alloc()
 */
template <typename DTYPE>
NeuralNetwork<DTYPE>::NeuralNetwork() : Module<DTYPE>()
{
#ifdef __DEBUG__
 ...
 * @details 동적으로 할당 받은 NeuralNetwork 클래스의 멤버 변수들을 할당 해제하고 클래스를
 * 소멸시킨다.
 * @return 없음
 ...
 */
template <typename DTYPE>
LossFunction<DTYPE>* NeuralNetwork<DTYPE>::SetLossFunction(LossFunction<DTYPE>* pLossFunction)
{
 ...
 */
template <typename DTYPE>
Optimizer<DTYPE>* NeuralNetwork<DTYPE>::SetOptimizer(Optimizer<DTYPE>* pOptimizer)
{
...
template <typename DTYPE>
Operator<DTYPE>* NeuralNetwork<DTYPE>::GetResultOperator()
{
    return this->GetExcutableOperatorContainer()->GetLast();
}

template <typename DTYPE>
Tensor<DTYPE>* NeuralNetwork<DTYPE>::GetResult()
{
...
template <typename DTYPE>
LossFunction<DTYPE>* NeuralNetwork<DTYPE>::GetLossFunction()
{
    return m_aLossFunction;
}

template <typename DTYPE>
Optimizer<DTYPE>* NeuralNetwork<DTYPE>::GetOptimizer()
{
    return m_aOptimizer;
}
 ...
 * @details NeuralNetwork의 Device 멤버변수를 CPU로 전환하고, Excutable Operator Container의 각
 * Operator들에서 Operator<DTYPE>::SetDeviceCPU() 메소드를 순서대로 호출하고, Lossfunction의
 * LossFunction<DTYPE>::SetDeviceCPU() 메소드를 호출한다.
 * @return 없음
 */
template <typename DTYPE>
void NeuralNetwork<DTYPE>::SetDeviceCPU()
{
    if (m_Device != CPU)
        this->SetDeviceCPUOnNeuralNetwork();
}

template <typename DTYPE>
void NeuralNetwork<DTYPE>::SetDeviceCPUOnNeuralNetwork()
{
    m_Device = CPU;
 ...
 * @details NeuralNetwork의 Device 멤버 변수를 확인하여 CPU 시
 * NeuralNetwork<DTYPE>::TrainingOnCPU()을 호출하고, GPU 시 NeuralNetwork<DTYPE>::TrainingOnGPU()을
 * 호출한다.
 * @return 성공 시 TRUE, m_Device 멤버 변수가 잘못된 값을 갖고 있을 때 FALSE를 반환한다.
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::Train()
{
    if (m_Device == CPU)
    {
        this->TrainOnCPU();
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTT(int timesize)
{
    if (m_Device == CPU)
    {
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTT(int timesize, int truncated_size)
{
    if (m_Device == CPU)
    {
 ...
 * @details NeuralNetwork의 Device 멤버 변수를 확인하여 CPU 시
 * NeuralNetwork<DTYPE>::TestingOnCPU()을 호출하고, GPU 시 NeuralNetwork<DTYPE>::TestingOnGPU()을
 * 호출한다.
 * @return 성공 시 TRUE, m_Device 멤버 변수가 잘못된 값을 갖고 있을 때 FALSE를 반환한다.
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::Test()
{
    if (m_Device == CPU)
    {
        this->TestOnCPU();
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTT_Test(int timesize)
{
    if (m_Device == CPU)
    {
 ...
 * @details 순서대로 Excutable Operator들의 Result와 Gradient를 초기화하고 Loss Function의 Result와
 * Gradient를 초기화하고 ForwardPropagate, BackwardPropagate 메소드를 호출하고 Optimizer로
 * 파라미터를 학습시킨다.
 * @details 각 메소드 참조
 * @return TRUE
 * @see NeuralNetwork<DTYPE>::ResetOperatorResult() NeuralNetwork<DTYPE>::ResetOperatorGradient()
 * NeuralNetwork<DTYPE>::ResetLossFunctionResult() NeuralNetwork<DTYPE>::ResetLossFunctionGradient()
 * @see NeuralNetwork<DTYPE>::ForwardPropagate() NeuralNetwork<DTYPE>::BackPropagate()
 * Optimizer<DTYPE>::UpdateParameter()
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::TrainOnCPU()
{
    this->ResetResult();
    this->ResetGradient();
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTTOnCPU(int timesize)
{
    this->ResetResult();
    this->ResetGradient();
    this->ResetLossFunctionResult();
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTTOnCPU(int timesize, int truncated_size)
{
...
template <typename DTYPE>
int NeuralNetwork<DTYPE>::BPTT_TestOnCPU(int timesize)
{
 ...
 * @details 순서대로 Excutable Operator들의 Result를 초기화하고 Loss Function의 Result를 초기화하고
 * ForwardPropagate메소드를 호출한다.
 * @details 각 메소드 참조
 * @return TRUE
 * @see NeuralNetwork<DTYPE>::ResetOperatorResult() NeuralNetwork<DTYPE>::ResetLossFunctionResult()
 * NeuralNetwork<DTYPE>::ForwardPropagate()
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::TestOnCPU()
{
    // this->ResetOperatorResult();
    this->ResetResult();
    this->ResetLossFunctionResult();
 ...
 * @details 순서대로 Excutable Operator들의 Result와 Gradient를 초기화하고 Loss Function의 Result와
 * Gradient를 초기화하고
 * @detaisl ForwardPropagateOnGPU, BackwardPropagateOnGPU 메소드를 호출하고 Optimizer로 파라미터를
 * 학습시킨다.
 * @details 각 메소드 참조
 * @return TRUE
 * @see NeuralNetwork<DTYPE>::ResetOperatorResult() NeuralNetwork<DTYPE>::ResetOperatorGradient()
 * NeuralNetwork<DTYPE>::ResetLossFunctionResult() NeuralNetwork<DTYPE>::ResetLossFunctionGradient()
 * @see NeuralNetwork<DTYPE>::ForwardPropagateOnGPU() NeuralNetwork<DTYPE>::BackPropagateOnGPU()
 * Optimizer<DTYPE>::UpdateParameterOnGPU()
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::TrainOnGPU()
{
#ifdef __CUDNN__
    // this->ResetOperatorResult();
 ...
 * @details 순서대로 Excutable Operator들의 Result를 초기화하고 Loss Function의 Result를 초기화하고
 * ForwardPropagateOnGPU메소드를 호출한다.
 * @details 각 메소드 참조
 * @return TRUE
 * @see NeuralNetwork<DTYPE>::ResetOperatorResult() NeuralNetwork<DTYPE>::ResetLossFunctionResult()
 * NeuralNetwork<DTYPE>::ForwardPropagateOnGPU()
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::TestOnGPU()
{
#ifdef __CUDNN__
    // this->ResetOperatorResult();
    this->ResetResult();
 ...
 */
template <typename DTYPE>
float NeuralNetwork<DTYPE>::GetAccuracy(int numOfClass)
{
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::GetMaxIndex(Tensor<DTYPE>* data, int ba, int ti, int numOfClass)
{
 ...
 */
template <typename DTYPE>
float NeuralNetwork<DTYPE>::GetTop5Accuracy(int numOfClass)
{
    Operator<DTYPE>* result = GetResultOperator();
    Operator<DTYPE>* label = m_aLossFunction->GetLabel();
...
/*!
 * @brief Tensor의 LongArray의 Element들 중 가장 큰 다섯 개 값에 대한 인덱스를 계산해 반환하는
 * 메소드
 * @param data 탐색하고자 하는 Tensor
 * @param ba Tensor의 batch Size
 * @param ti Tensor의 Time Size
 ...
 */
template <typename DTYPE>
void NeuralNetwork<DTYPE>::GetTop5Index(Tensor<DTYPE>* data, int* top5Index, int ba, int ti,
                                        int numOfClass)
{
    Shape* pShape = data->GetShape();
    int start = Index5D(pShape, ti, ba, 0, 0, 0);
 ...
 */
template <typename DTYPE>
float NeuralNetwork<DTYPE>::GetLoss()
{
    float avg_loss = 0.f;
...
// loss값 잘 갖고옴!!!
    ...
    for (int ti = 0; ti < timesize; ti++)
    {
        for (int ba = 0; ba < batchsize; ba++)
        {
            ...
            // avg_loss += (*m_aLossFunction)[batchsize*ti + ba] / batchsize / timesize; //뭐가
            // 맞는걸까... avg_loss += (*m_aLossFunction)[batchsize*ba + ti] ;/// batchsize /
            // timesize;
        }
    }
 ...
 */
template <typename DTYPE>
void NeuralNetwork<DTYPE>::PrintGraphInformation()
{
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::ResetLossFunctionResult()
{
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::ResetLossFunctionGradient()
{
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::ResetParameterGradient()
{
...
template <typename DTYPE>
Operator<DTYPE>* NeuralNetwork<DTYPE>::SearchOperator(std::string pName)
{
    ...
    for (int i = 0; i < this->GetNumOfExcutableOperator(); i++)
    {
        ...
        if (name == pName)
            return (*this->GetExcutableOperatorContainer())[i];
    }
...
/*
template<typename DTYPE> void NeuralNetwork<DTYPE>::InputToFeature(int inDim, int noSample, float
*pSamples[], int outDim, float *pFeatures[], int batchSize)
{
#ifdef  __DEBUG__
    ...
    //printf("Starting %s, inDim = %d, outDim = %d, %lu samples\n", __FUNCTION__, inDim, outDim,
vSample.size()); int noBatch = (noSample + batchSize - 1) / batchSize;
...
//    vFeature.resize(noSample);
    ...
    for(int batchIdx  = 0; batchIdx < noBatch; batchIdx++){
        ...
        for(int idx = 0; idx < curBatch; idx++)
            memcpy(&(*input)[idx * inDim], pSamples[batchIdx * batchSize + idx], inDim *
sizeof(float));
...
__LINE__);
            ...
            memcpy(pFeatures[batchIdx * batchSize + idx], &(*result)[idx * outDim], outDim *
sizeof(float));
 ...
 * @details 파라미터로 전달받은 GPU 장치 번호에 해당하는 GPU에 메모리를 할당하고 NeuralNetwork의
 * Device 멤버변수를 GPU로 전환한다
 * @details Excutable Operator Container, Parameter Operator Container, Input Operator Container의
 * 각 Operator들에서 Operator<DTYPE>::SetDeviceGPU(cudnnHandle_t& pCudnnHandle, unsigned int
 * idOfDevice) 메소드를 순서대로 호출한다
 * @details Lossfunction의 LossFunction<DTYPE>::SetDeviceGPU(cudnnHandle_t& pCudnnHandle, unsigned
 * int idOfDevice) 메소드를 호출하고, Optimizer의 (cudnnHandle_t& pCudnnHandle, unsigned int
 * idOfDevice) 메소드를 호출한다.
 * @param idOfDevice 학습에 이용하려는 GPU 장치 번호
 * @return 없음
 */
template <typename DTYPE>
void NeuralNetwork<DTYPE>::SetDeviceGPU(unsigned int idOfDevice)
{
    if (m_Device != GPU)
    {
...
template <typename DTYPE>
void NeuralNetwork<DTYPE>::SetDeviceGPUOnNeuralNetwork(cudnnHandle_t& pCudnnHandle,
                                                       unsigned int idOfDevice)
{
    if (m_aLossFunction != NULL)
        m_aLossFunction->SetDeviceGPU(m_cudnnHandle, idOfDevice);
    if (m_aOptimizer != NULL)
 ...
 */
template <typename DTYPE>
int NeuralNetwork<DTYPE>::SetDeviceID(unsigned int idOfDevice)
{
