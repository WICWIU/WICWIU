...
template <typename DTYPE>
void NeuralNetwork<DTYPE>::InputToFeature(int inDim, int noSample, float* pSamples[], int outDim,
                                          float* pFeatures[], int batchSize)
{
#ifdef __DEBUG__
    ...
    for (int batchIdx = 0; batchIdx < noBatch; batchIdx++)
    {
        ...
        for (int idx = 0; idx < curBatch; idx++)
            memcpy(&(*input)[idx * inDim], pSamples[batchIdx * batchSize + idx],
                   inDim * sizeof(float));
        ...
        for (int idx = 0; idx < curBatch; idx++)
        {
            ...
            memcpy(pFeatures[batchIdx * batchSize + idx], &(*result)[idx * outDim],
                   outDim * sizeof(float));
