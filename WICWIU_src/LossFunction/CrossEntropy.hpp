...
@details Cross Entropy 계산식을 이용해 뉴럴 네트워크의 순전파를 통해 계산된 출력 Tensor와 레이블
값의 손실 함수를 계산한다
*/
...
public:
    ...
    CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon) 메소드를 호출한다.
    @param pOperator CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon) 메소드의
    매개변수로 전달할 Operator
    ...
    */
    CrossEntropy(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, int epsilon = 1e-6f)
    ...
    매개변수로 전달하여 CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon) 메소드를
    호출한다.
    @param pOperator CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon) 메소드의
    매개변수로 전달할 Operator
    @param pLabel LossFunction의 입력 레이블에 해당하는 Operator
    ...
    @see CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon)
    */
    CrossEntropy(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, std::string pName)
    ...
    @see CrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, int epsilon)
    */
    CrossEntropy(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, int epsilon,
                 std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout
            << "CrossEntropy::CrossEntropy(Operator<DTYPE> *, Operator<DTYPE> *, int, std::string)"
            << '\n';
#endif // __DEBUG__
        this->Alloc(pOperator, epsilon);
    }
    ...
    */
    ...
    @details 구성한 뉴럴 네트워크에서 얻어진 CrossEntropy LossFunction에 대한 입력 Tensor의
    Gradient를 계산한다
    @param pTime 입력 Tensor의 Time 축의 Dimension
    @return NULL
    */
