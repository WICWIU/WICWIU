...
public:
    VanillaGANDiscriminatorLoss(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, DTYPE epsilon,
                                std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout << "VanillaGANDiscriminatorLoss::VanillaGANDiscriminatorLoss(Operator<DTYPE> *, "
    ...
    VanillaGANDiscriminatorLoss(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel,
                                std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        ...
        for (int ba = 0; ba < batchsize; ba++)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                sumOfLossBatches += -(*label)[i] * log((*input)[i] + m_epsilon) -
                                    (1 - (*label)[i]) * log(1.0 - (*input)[i] + m_epsilon);
            }
        }
        // std::cout << "(*label)[i] : " << (*label)[0] << '\n';
        // std::cout << "(*input)[0] : " << (*input)[0] << ", ";
        // std::cout << "(*label)[i] * log((*input)[i] + m_epsilon) : " << (*label)[0] *
        // log((*input)[0] + m_epsilon) << '\n'; std::cout << "(1 - (*label)[i]) * log(1.0 -
        // (*input)[i] + m_epsilon) : " << (1 - (*label)[0]) * log(1.0 - (*input)[0] + m_epsilon) <<
        // '\n'; std::cout << "(*label)[i] * log((*input)[i] + m_epsilon) + (1 - (*label)[i]) *
        // log(1.0 - (*input)[i] + m_epsilon) : " << (*label)[0] * log((*input)[0] + m_epsilon) + (1
        // - (*label)[0]) * log(1.0 - (*input)[0] + m_epsilon) << '\n'; std::cout <<
        // "sumOfLossBatches : " << sumOfLossBatches << '\n';
        ...
        for (int ba = 0; ba < batchsize; ba++)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                (*input_delta)[i] += -((*label)[i] / ((*input)[i] + m_epsilon) -
                                       (1 - (*label)[i]) / (1.0 - ((*input)[i] + m_epsilon)));
            }
        }
    }
