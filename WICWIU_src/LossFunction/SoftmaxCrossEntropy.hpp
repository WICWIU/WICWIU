...
@details Cross Entropy 계산 식을 이용해 뉴럴 네트워크의 순전파를 통해 계산된 출력 Tensor와 레이블
값의 손실 함수를 계산한다
@details Softmax Function을 뉴럴 네트워크의 마지막 Operator로 사용해 뉴럴 네트워크의 Gradient 계산을
용이하게 한다
*/
...
public:
    ...
    @details LossFunction 클래스의 생성자를 호출하고, Operator와 epsilon을 매개변수로 전달하여
    SoftmaxCrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon) 메소드를 호출한다.
    @param pOperator SoftmaxCrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon)
    메소드의 매개변수로 전달할 Operator
    ...
    */
    SoftmaxCrossEntropy(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, DTYPE epsilon,
                        std::string pName = "NO NAME")
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout
            << "SoftmaxCrossEntropy::SoftmaxCrossEntropy(Operator<DTYPE> *, Operator<DTYPE> *, int)"
            << '\n';
#endif // __DEBUG__
    ...
    /*!
    @brief SoftmaxCrossEntropy LossFunction 클래스 생성자
    @details LossFunction 클래스의 생성자를 호출하고, Operator와 1e-6f에 해당하는 epsilon 값을
    매개변수로 전달하여 SoftmaxCrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon)
    메소드를 호출한다.
    @param pOperator SoftmaxCrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon)
    메소드의 매개변수로 전달할 Operator
    @param pLabel LossFunction의 입력 레이블에 해당하는 Operator
    ...
    @see SoftmaxCrossEntropy<DTYPE>::Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon)
    */
    SoftmaxCrossEntropy(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel,
                        std::string pName = "NO NAME")
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
    ...
    */
    ...
    @details SoftmaxCrossEntropy 클래스의 멤버 변수들의 값을 초기화 하고 포인터 멤버 변수들에
    동적으로 메모리를 할당한다.
    ...
    */
    ...
    @details LossFunction의 입력 Operator의 Tensor 값에 대해서 softmax 함수 값을 계산하고 이를
    레이블 값과 비교해 Cross Entropy를 구한다
    ...
    */
    Tensor<DTYPE>* ForwardPropagate(int pTime = 0)
    {
        ...
        for (int ba = 0; ba < batchsize; ba++)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                (*result)[ti * batchsize + ba] +=
                    -(*label)[i] * log((*softmaxresult)[i] + m_epsilon);
            }
        }
    ...
    @details 구성한 뉴럴 네트워크에서 얻어진 Softmax CrossEntropy LossFunction에 대한 입력 Tensor의
    Gradient를 계산한다
    @param pTime 입력 Tensor의 Time 축의 Dimension
    @return NULL
    */
    ...
    //
    // checkCUDNN(cudnnSoftmaxForward(this->GetCudnnHandle(), CUDNN_SOFTMAX_ACCURATE,
    // CUDNN_SOFTMAX_MODE_INSTANCE, &alpha, pInputDesc, pDevInput, &beta, pSoftMaxDesc,
    // pDevSoftMax));
    //
    // int start = 0;
    // int end   = 0;
