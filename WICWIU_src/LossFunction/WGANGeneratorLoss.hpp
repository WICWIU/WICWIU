...
{
public:
    WGANGeneratorLoss(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout << "WGANGeneratorLoss::WGANGeneratorLoss(Operator<DTYPE> *, MetaParameter *, "
                     "std::string)"
                  << '\n';
#endif // __DEBUG__
