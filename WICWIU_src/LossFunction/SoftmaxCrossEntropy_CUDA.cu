...
__global__ void SoftmaxCrossEntropy_ForwardPropagate_kernel(int time, int batchsize, int colsize,
                                                            float epsilon, float* result,
                                                            float* label, float* softmaxresult)
{
    int result_idx = 0;
    int start = 0;
    int end = 0;

    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < batchsize;
...
Tensor<DTYPE>* SoftmaxCrossEntropy<DTYPE>::ForwardPropagateOnGPU(int pTime)
{
    ...
    checkCUDNN(cudnnSoftmaxForward(this->GetCudnnHandle(), CUDNN_SOFTMAX_ACCURATE,
                                   CUDNN_SOFTMAX_MODE_INSTANCE, &alpha, pInputDesc, pDevInput,
    ...
    SoftmaxCrossEntropy_ForwardPropagate_kernel<<<noBlock, threadsPerBlock>>>(
        pTime, batchsize, colsize, m_epsilon, pDevResult, pDevLabel, pDevSoftMax);

    return result;
}

__global__ void SoftmaxCrossEntropy_BackPropagate_kernel(int time, int capacity, float* input_delta,
                                                         float* label, float* softmaxresult)
{
    int idx = 0;

    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < capacity;
         idx += blockDim.x * gridDim.x)
    {
...
template <typename DTYPE>
Tensor<DTYPE>* SoftmaxCrossEntropy<DTYPE>::BackPropagateOnGPU(int pTime)
{
    Tensor<DTYPE>* label = this->GetLabel()->GetResult();
    Tensor<DTYPE>* softmaxresult = m_aSoftmaxResult;
    ...
    SoftmaxCrossEntropy_BackPropagate_kernel<<<noBlock, threadsPerBlock>>>(
        pTime, capacity, pDevInputDelta, pDevLabel, pDevSoftMax);
