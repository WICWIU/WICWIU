...
public:
    VanillaGANGeneratorLoss(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, DTYPE epsilon,
                            std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout << "VanillaGANGeneratorLoss::VanillaGANGeneratorLoss(Operator<DTYPE> *, "
                     "Operator<DTYPE> *, MetaParameter *, std::string)"
                  << '\n';
#endif // __DEBUG__
        this->Alloc(pOperator, epsilon);
    }

    VanillaGANGeneratorLoss(Operator<DTYPE>* pOperator, Operator<DTYPE>* pLabel, std::string pName)
        : LossFunction<DTYPE>(pOperator, pLabel, pName)
    {
#ifdef __DEBUG__
        std::cout << "VanillaGANGeneratorLoss::VanillaGANGeneratorLoss(Operator<DTYPE> *, "
