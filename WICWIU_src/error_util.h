...
#define COMPILER_VER                                                                               \
    TOSTR(__clang_major__) "." TOSTR(__clang_minor__) "." TOSTR(__clang_patchlevel__)
...
static void showDevices(void)
{
    ...
    for (int i = 0; i < totalDevices; i++)
    {
        ...
        printf("device %d : sms %2d  Capabilities %d.%d, SmClock %.1f Mhz, MemSize (Mb) %d, "
               "MemClock %.1f Mhz, Ecc=%d, boardGroupID=%d\n",
               i, prop.multiProcessorCount, prop.major, prop.minor, (float)prop.clockRate * 1e-3,
               (int)(prop.totalGlobalMem / (1024 * 1024)), (float)prop.memoryClockRate * 1e-3,
               prop.ECCEnabled, prop.multiGpuBoardGroupID);
    }
}
