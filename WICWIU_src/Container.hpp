...
public:
    ...
    */
    ...
    @details 기존의 queue를 할당 해제하고 매개변수로 받은 Element를 마지막에 추가하여 새로운 Queue를
    동적으로 할당한다.
    ...
    */
    ...
    @details 기존의 queue를 할당 해제하고 Queue의 첫번째 Element를 반환한 후 새로운 Queue를 동적으로
    할당한다.
    ...
    */
    ...
    @details 매개변수로 받은 Element가 Queue에 존재할 경우, 해당 Element를 반환하고 Queue를 새로
    동적으로 할당한다.
    ...
    */
    ...
    @details Queue의 Element를 반대 순서로 저장하는 새로운 Queue를 할당하고, 기존의 Queue를 할당
    해제한다.
    @return TRUE
    */
    int Reverse()
    ...
    DTYPE* GetRawData() const { return m_aElement; }

    DTYPE GetElement(unsigned int index) { return m_aElement[index]; }
    ...
    */
    DTYPE& operator[](unsigned int index) { return m_aElement[index]; }
};
