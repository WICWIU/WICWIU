...
@details Shape 클래스 동적 할당에 사용했던 메모리 공간을 반환하고 해당 메모리 공간을 NULL로
초기화한다.
@return 없음
*/
void Shape::Delete()
...
@details cuda와 cudnn 라이브러리를 이용하여, 매개변수로 받은 GPU 번호에 해당되는 GPU의 메모리 공간에
Descriptor 변수를 할당한다
...
@details cudnn 라이브러리를 사용하여, Descriptor 동적 할당에 사용했던 메모리 공간을 반환하고 해당
메모리 공간을 NULL로 초기화한다.
@return 없음
*/
...
@details 사전에 정의 한 DeleteOnGPUdhk AllocOnGPU 메소드를 이용하여, 기존에  GPU에 할당 되어있던
Descriptor를 할당 해제 시키, 새로운 Descriptor를 할당 시킨다.
@return 성공 시 TRUE.
*/
...
@return m_idOfDevice
*/
int Shape::GetDeviceID() { return m_idOfDevice; }
...
@details CPU에서 GPU로 전환 시, 매개변수로 받은 번호에 해당하는 GPU의 메모리에 필요한 공간을
동적으로 할당한다.
...
@details cudnn의 Tensor Descriptor, GPU 연산에서 사용하는 Tensor의 shape 정보를 담고 있는 포인터
변수를 반환한다.
