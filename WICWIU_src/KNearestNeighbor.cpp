...
KNearestNeighbor::KNearestNeighbor(int dim, int noClass, int noRef, int *pRefLabel, float *pRefVector[])
{
    try{
        m_vRefLabel.resize(noRef);
        m_vRefVector.resize(noRef);
    } catch(...){
        printf("Failed to allocate memory (dim = %d, noClass = %d) in %s (%s %d)\n", dim, noClass, __FUNCTION__, __FILE__, __LINE__);
        return;
    }
...
void KNearestNeighbor::AddReference(int label, float *pRefVector)
{
    ...
    if(newRef == NULL){
        printf("Failed to allocate memory (dim = %d, noClass = %d) in %s (%s %d)\n", m_dim, m_noClass, __FUNCTION__, __FILE__, __LINE__);
        return;
    }
    ...
    for(i = 0; i < noRef; i++){
        ...
        if(p < k){
            if(p == vMinDist2.size()){
                ...
                memmove(&vMinDist2[p+1], &vMinDist2[p], (vMinDist2.size() - p - 1) * sizeof(vMinDist2[0]));
                vMinDist2[p] = dist2;

                if(vMinLabel.size() < k)
                    vMinLabel.resize(vMinLabel.size() + 1);
                memmove(&vMinLabel[p+1], &vMinLabel[p], (vMinLabel.size() - p - 1) * sizeof(vMinLabel[0]));
...
void knn_test()
{
    ...
    float aaRef[10][5] = {
        ...
        { 0.F, 0.F, 0.F, 1.F, 0.F },
        { 0.F, 0.F, 0.F, 0.F, 1.F },
        { 1.1F, 0.F, 0.F, 0.F, 0.F },
        { 0.F, 1.1F, 0.F, 0.F, 0.F },
        { 0.F, 0.F, 1.1F, 0.F, 0.F },
        { 0.F, 0.F, 0.F, 1.1F, 0.F },
        { 0.F, 0.F, 0.F, 0.F, 1.1F }
    };
    ...
    float aaTest[3][5]= {
        { 0.7F, 0.1F, 0.F, 0.F, 0.F },
        { 0.F, 0.1F, 0.8F, 0.F, 0.F },
        { 0.F, 0.F, 0.F, 0.F, 1.2F }
    };
