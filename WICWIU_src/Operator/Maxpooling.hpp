...
public:
    ...
    */
    Maxpooling2D(Operator<DTYPE>* pInput, int maskRow, int maskCol, int strideRow, int strideCol,
    ...
    */
    Maxpooling2D(Operator<DTYPE>* pInput, int maskRow, int maskCol, int strideRow, int strideCol,
                 int padding, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
    ...
    */
              ...
              int padding1 = 0, int padding2 = 0)
    {
        ...
        indexOfMaxInput = new Tensor<int>((*shapeOfInput)[0], (*shapeOfInput)[1],
                                          (*shapeOfInput)[2], rowsize, colsize);
    ...
    @details TensorDesriptor들을 생성하고, TensorDesriptor들으 데이터가 batch, channel, row, col
    순서로 배치되도록 지정한다.
    ...
    */
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnSetPooling2dDescriptor(
            m_aPoolingDesc, CUDNN_POOLING_MAX, CUDNN_PROPAGATE_NAN, m_mask[0], m_mask[1],
            m_padding[0], m_padding[1], m_stride[0], m_stride[1]));

        checkCUDNN(cudnnSetTensor4dDescriptor(m_aInputTensorDesc, CUDNN_TENSOR_NCHW,
                                              CUDNN_DATA_FLOAT, batchsize, channelsize,
                                              rowsizeOfInput, colsizeOfInput));
        ...
        checkCUDNN(cudnnGetPooling2dForwardOutputDim(m_aPoolingDesc, m_aInputTensorDesc, &batchsize,
                                                     &channelsize, &rowsize, &colsize));
    }
    ...
    */
    ...
    @details Filter(temprow * tempcol)의 범위 중 가장 큰 값을 result에 저장하고, index는
    indexOfMaxInput에 저장한다.
            ...
            { // Batchsize of weight kernel
                for (int ro = 0; ro < rowsize; ro++)
                {
                    for (int co = 0; co < colsize; co++)
                    {
                        ...
                        (*input_delta)[(*indexOfMaxInput)[indexOfDelta]] +=
                            (*this_delta)[indexOfDelta];
                    }
                }
            }
        }
