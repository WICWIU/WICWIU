...
public:
    ...
    */
    Softmax(Operator<DTYPE>* pOperator, DTYPE epsilon = 1e-6f, int pLoadflag = TRUE)
        : Operator<DTYPE>(pOperator, pLoadflag)
    {
#ifdef __DEBUG__
    ...
    @ref virtual int Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon = 1e-6f
    */
    Softmax(Operator<DTYPE>* pOperator, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pOperator, pName, pLoadflag)
    {
#ifdef __DEBUG__
        std::cout << "Softmax::Softmax(Operator *)" << '\n';
#endif // __DEBUG__
    ...
    @ref virtual int Alloc(Operator<DTYPE> *pOperator, DTYPE epsilon = 1e-6f
    */
    Softmax(Operator<DTYPE>* pOperator, DTYPE epsilon, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pOperator, pName, pLoadflag)
    {
#ifdef __DEBUG__
        std::cout << "Softmax::Softmax(Operator *)" << '\n';
    ...
    */
    ...
    @details input으로 받은 Operator의 Shape정보들로 맴버 변수드을 초기화 하고, 같은 Shape을 갖는
    Tensor를 만들어 Result와 Gradient로 설정한다.
    ...
    */
    ...
    @details max값을 계산하고, exp()한 모든 값들을 더해 sum을 구한 뒤, 각각의 exp(input)한 값을
    sum으로 나누어주어 확률값을 구하고 result에 저장한다.
