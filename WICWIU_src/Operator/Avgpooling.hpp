...
class GlobalAvaragePooling2D : public Operator<DTYPE>
{
private:
    int m_timesize;
    ///< time의 dimension 크기
...
public:
    ...
    */
    GlobalAvaragePooling2D(Operator<DTYPE>* pInput, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
        std::cout
            << "GlobalAvaragePooling2D::GlobalAvaragePooling2D(Operator<DTYPE> *, std::string)"
            << '\n';
#endif // __DEBUG__
        Alloc(pInput);
    }
    ...
    */
    int Alloc(Operator<DTYPE>* pInput)
    {
        ...
        this->AddGradient(
            new Tensor<DTYPE>(new Shape(m_timesize, m_batchsize, m_channelsize, 1, 1)));
    ...
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnSetPooling2dDescriptor(m_aPoolingDesc,
                                               CUDNN_POOLING_AVERAGE_COUNT_EXCLUDE_PADDING,
                                               CUDNN_PROPAGATE_NAN, m_rowsize, m_colsize, // mask
                                               0, 0,                                      // padding
    ...
    */
    ...
    {
        Container<Operator<DTYPE>*>* input_contatiner = this->GetInputContainer();
        Container<Tensor<DTYPE>*>* input_gradient_container =
            (*input_contatiner)[0]->GetGradientContainer();
    ...
    int ForwardPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnPoolingForward(this->GetCudnnHandle(), m_aPoolingDesc, &m_alpha,
    ...
    int BackPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnPoolingBackward(this->GetCudnnHandle(), m_aPoolingDesc, &m_alpha,
                                        m_aOutputTensorDesc, m_pDevOutput, m_aDeltaDesc,
                                        m_pDevDelta, m_aInputTensorDesc, m_pDevInput, &m_alpha,
                                        m_aInputDeltaDesc, m_pDevInputDelta));
...
template <typename DTYPE>
class AvaragePooling2D : public Operator<DTYPE>
{
private:
    int m_stride[2];
    int m_mask[2];
...
public:
    AvaragePooling2D(Operator<DTYPE>* pInput, int maskRow, int maskCol, int strideRow,
                     int strideCol, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
    ...
    AvaragePooling2D(Operator<DTYPE>* pInput, int maskRow, int maskCol, int strideRow,
                     int strideCol, int padding, std::string pName, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
              ...
              int padding1 = 0, int padding2 = 0)
    {
        ...
        this->SetResult(new Tensor<DTYPE>((*shapeOfInput)[0], (*shapeOfInput)[1],
                                          (*shapeOfInput)[2], rowsize, colsize));
        this->SetDelta(new Tensor<DTYPE>((*shapeOfInput)[0], (*shapeOfInput)[1], (*shapeOfInput)[2],
                                         rowsize, colsize));
    ...
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnSetPooling2dDescriptor(
            m_aPoolingDesc, CUDNN_POOLING_AVERAGE_COUNT_EXCLUDE_PADDING, CUDNN_PROPAGATE_NAN,
