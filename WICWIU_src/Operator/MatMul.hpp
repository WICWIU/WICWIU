...
private:
    ...
    // DTYPE *m_pHostInput, *m_pHostOutput, *m_pHostFilter, *m_pHostInputDelta, *m_pHostDelta,
    // *m_pHostFilterDelta;
...
public:
    ...
    */
    MatMul(Operator<DTYPE>* pWeight, Operator<DTYPE>* pInput, std::string pName,
           int pLoadflag = TRUE)
        : Operator<DTYPE>(pWeight, pInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
    ...
    */
    ...
    @details timesize, batchsize, channelsize, row_size는 pInput의 Shape과 같게,  colsize는
    pWeight와 같게 초기화한다.
    @details input x weight을 하기 때문에 rowsize는 pInput의 Shape을, colsize는 pWeight의 Shape을
    받는다.
    @details Result와 Delta를 저장하기 위해 input의 rowsize, weight의 colsize를 갖는 Tensor를
    생성한다.
    @param pWeight MatMul할 weight.
    @param pInput Matmul할 input Operator.
    @return 성공 시 TRUE.
    ...
    */
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnGetConvolutionBackwardFilterWorkspaceSize(
            this->GetCudnnHandle(), inputTensorDesc, deltaDesc, convDesc, filterDesc, m_filterAlgo,
            &m_filterSizeInBytes));
    ...
    @details m_devWorkSpace, m_dataDevWorkSpace, m_filterDevWorkSpace들이 가리키는 메모리를
    해제한다.
    */
    void Delete()
    {
    ...
    */
    ...
    @details cudnn이 제공하는 Convolution ForwardPropagate 메소드를 이용하여 MatMul의
    ForwardPropagate연산을 한다.
    ...
    */
    ...
    @details cudnn이 제공하는 Convolution BackwardPropagate 메소드를 아용하여 MatMul의
    BackwardPropagate연산을 한다.
