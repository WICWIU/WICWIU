...
class Dropout : public Operator<DTYPE>
{
private:
    float m_droprate;
    Mode m_mode;
    ...
    DTYPE *m_pDevInput, *m_pDevOutput, *m_pDevInputDelta, *m_pDevDelta, *m_pRandNumGenerator,
        *m_pReserveSpace;
...
public:
    ...
    Dropout(Operator<DTYPE>* pInput, float pDroprate, std::string pName, int pLoadflag = TRUE)
    ...
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnSetDropoutDescriptor(m_aDropoutDesc, this->GetCudnnHandle(), m_droprate,
                                             m_pRandNumGenerator, m_dataSizeInBytes, time(NULL)));
    }
    ...
    int ForwardPropagateOnGPU(int pTime = 0)
    {
        ...
        switch (m_mode)
        {
            case TRAIN:
                ...
                checkCUDNN(cudnnDropoutForward(this->GetCudnnHandle(), m_aDropoutDesc, m_aInOutDesc,
                                               m_pDevInput, m_aInOutDesc, m_pDevOutput,
                                               m_pReserveSpace, m_spaceSizeInBytes));

                break;
            case INFERENCE:

                checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &alpha, m_aInOutDesc, m_pDevInput,
                                          &alpha, m_aInOutDesc, m_pDevOutput));
                break;
        }
    ...
    int BackPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnDropoutBackward(this->GetCudnnHandle(), m_aDropoutDesc, m_aInOutDesc,
                                        m_pDevDelta, m_aInOutDesc, m_pDevInputDelta,
                                        m_pReserveSpace, m_spaceSizeInBytes));
