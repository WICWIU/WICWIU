...
class PRelu : public Operator<DTYPE>
{
private:
#ifdef __CUDNN__
...
public:
    ...
    */
    PRelu(Operator<DTYPE>* pInput, Operator<DTYPE>* pWeight, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pWeight, "NO NAME", pLoadflag)
    {
#ifdef __DEBUG__
    ...
    @ref int Alloc(Operator<DTYPE> *pInput, Operator<DTYPE> *pWeight)
    */
    PRelu(Operator<DTYPE>* pInput, Operator<DTYPE>* pWeight, std::string pName,
          int pLoadflag = TRUE)
    ...
    */
    int BackPropagate(int pTime = 0)
    {
            ...
            for (int ch = 0; ch < channelsize; ch++)
            {
                for (int ro = 0; ro < rowsize; ro++)
                {
                    for (int co = 0; co < colsize; co++)
                    {
                        ...
                        /*
                        if(std::isnan((*weight)[index]) || std::isnan((*this_delta)[index]) ||
                        std::isnan((*input_delta)[index]) || std::isnan((*weight_delta)[index])){
                          std::cin >> x;
                        }*/
                    }
                }
        ...
        for(; input_delta_ptr < delta_limit; input_delta_ptr++, this_delta_ptr++, input_ptr++,
        weight_ptr++, result_ptr++){ if(*result_ptr > 0.f) *input_delta_ptr += *this_delta_ptr; else
