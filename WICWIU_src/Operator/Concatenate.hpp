...
public:
    ConcatenateChannelWise(Operator<DTYPE>* pInput0, Operator<DTYPE>* pInput1,
    ...
    int Alloc(int noOperator, ...)
    {
        ...
        for (int i = 1; i < noOperator; i++)
        {
            ...
            if (i != noOperator - 1)
                m_aAccumulate[i + 1] = totalchannelsize;
        }
            ...
            for (int ba = 0; ba < batchsize; ba++)
            {
                for (int ch = 0; ch < channelsize; ch++)
                {
                    for (int ro = 0; ro < rowsize; ro++)
                    {
                        ...
                        {
                            (*result)[Index5D(resultTenShape, ti, ba, m_aAccumulate[opnum] + ch, ro,
                                              co)] =
                    ...
                    for (int ro = 0; ro < rowsize; ro++)
                    {
                        for (int co = 0; co < colsize; co++)
                        {
                            (*input_delta)[Index5D(inputTenShape, ti, ba, ch, ro, co)] +=
                                (*this_delta)[Index5D(resultTenShape, ti, ba,
                                                      m_aAccumulate[opnum] + ch, ro, co)];
                        }
                    }
                }
            }
