...
{
public:
    ReconstructionError(Operator<DTYPE>* pInput, Operator<DTYPE>* pLabel, std::string pName)
        : Operator<DTYPE>(pInput, pLabel, pName)
    {
#ifdef __DEBUG__
    ...
    int ForwardPropagate(int pTime = 0)
    {
        ...
        for (int j = 0, index = 0; j < capacity; j++)
        {
            ...
            // if(index < 784)
            // std::cout << "index =  " << index << ", input = " << (*input)[index] << ", label = "
            // << (*label)[index] << ", result = " << std::abs((*input)[index] - (*label)[index]) <<
            // "\n";
        }
    ...
    int BackPropagate(int pTime = 0)
    {
        ...
        for (int j = 0, index = 0; j < capacity; j++)
        {
            ...
            /*if((index < 784)&&(index % 100 == 0)){
                std::cout << "index = " << index <<  ", (*input)[index] = " << ((*input)[index] + 1)
             / 2 * 255 << ", (*label)[index] = " << ((*label)[index] + 1) / 2 * 255 << ",
             (*input)[index] - (*label)[index] = " << (*input)[index] - (*label)[index] << ",
             (*this_delta)[ti] = " << (*this_delta)[ti] << ", (*input_delta)[index] = " <<
             (*input_delta)[index] << ", (*label_delta)[index] = " <<(*label_delta)[index] << "\n";
             }*/
        }
        return TRUE;
    }
