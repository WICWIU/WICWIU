...
template<typename DTYPE> class GRUCell : public Operator<DTYPE>{
private:
...
public:
  GRUCell(Operator<DTYPE> *pInput, Operator<DTYPE> *pWeightIG, Operator<DTYPE> *pWeightHG, Operator<DTYPE> *pWeightICH, Operator<DTYPE> *pWeightHCH, Operator<DTYPE> *gBias, Operator<DTYPE> *chBias, Operator<DTYPE>* pInitHidden = NULL)
       : Operator<DTYPE>(7, pInput, pWeightIG, pWeightHG, pWeightICH, pWeightHCH, gBias, chBias) {
      #if __DEBUG__
      std::cout << "GRUCell::GRUCell(Operator<DTYPE> *)" << '\n';
      #endif  // __DEBUG__
      this->Alloc(pInput, pWeightIG, pWeightHG, pWeightICH, pWeightHCH, gBias, chBias, pInitHidden);
  }

    GRUCell(Operator<DTYPE> *pInput, Operator<DTYPE> *pWeightIG, Operator<DTYPE> *pWeightHG, Operator<DTYPE> *pWeightICH, Operator<DTYPE> *pWeightHCH, Operator<DTYPE> *gBias, Operator<DTYPE> *chBias, std::string pName, Operator<DTYPE>* pInitHidden = NULL)
...
void Delete() {}
    ...
    int ForwardPropagateOnGPU(int pTime = 0) {
        ...
        checkCUDNN(cudnnRNNForwardTraining(this->GetCudnnHandle(), rnn_desc, pTime+1,
                                           x_desc, x,
                                           hx_desc, hx,
                                           cx_desc, cx,
                                           w_desc, weights,
