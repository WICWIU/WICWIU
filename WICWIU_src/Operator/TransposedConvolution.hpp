...
class TransposedConvolution2D : public Operator<DTYPE>
{
private:
    int m_stride[2];
...
public:
    ...
    */
    TransposedConvolution2D(Operator<DTYPE>* pInput, Operator<DTYPE>* pWeight, int stride1,
                            int stride2, std::string pName = "NO NAME", int pLoadflag = TRUE)
    ...
    */
    TransposedConvolution2D(Operator<DTYPE>* pInput, Operator<DTYPE>* pWeight, int stride1,
                            int stride2, int padding, std::string pName = "NO NAME",
                            int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pWeight, pName, pLoadflag)
    {
    ...
    */
    TransposedConvolution2D(Operator<DTYPE>* pInput, Operator<DTYPE>* pWeight, int stride1,
                            int stride2, int padding1, int padding2, std::string pName = "NO NAME",
                            int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pWeight, pName, pLoadflag)
    {
    ...
    */
              ...
              int padding1, int padding2)
    {
        ...
        outputHeight =
            m_stride[0] * ((*shapeOfInput)[3] - 1) + (*shapeOfWeight)[3] - (2 * m_padding[0]);
        outputWidth =
            m_stride[1] * ((*shapeOfInput)[4] - 1) + (*shapeOfWeight)[4] - (2 * m_padding[1]);

        this->SetResult(new Tensor<DTYPE>((*shapeOfInput)[0], (*shapeOfInput)[1],
                                          (*shapeOfWeight)[2], outputHeight, outputWidth));
    ...
    순서로 배치되도록 지정한다.
    @details TransposedConvolution연산에 필요한 알고리즘을 정의하고, 연산에 필요한 메모리공간을 할당
    받는다.
    ...
    */
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnGetConvolutionForwardAlgorithm(
            this->GetCudnnHandle(), outputTensorDesc, filterDesc, convDesc, inputTensorDesc,
            CUDNN_CONVOLUTION_FWD_SPECIFY_WORKSPACE_LIMIT, 0, &m_algo));

        checkCUDNN(cudnnGetConvolutionForwardWorkspaceSize(this->GetCudnnHandle(), outputTensorDesc,
                                                           filterDesc, convDesc, inputTensorDesc,
                                                           m_algo, &m_sizeInBytes));

        checkCUDNN(cudnnGetConvolutionBackwardDataAlgorithm(
            this->GetCudnnHandle(), filterDesc, inputDeltaDesc, convDesc, deltaDesc,
            CUDNN_CONVOLUTION_BWD_DATA_SPECIFY_WORKSPACE_LIMIT, 0, &m_dataAlgo));

        checkCUDNN(cudnnGetConvolutionBackwardFilterAlgorithm(
            this->GetCudnnHandle(), deltaDesc, inputTensorDesc, convDesc, filterDeltaDesc,
        ...
        checkCUDNN(cudnnGetConvolutionBackwardDataWorkspaceSize(this->GetCudnnHandle(), filterDesc,
                                                                inputDeltaDesc, convDesc, deltaDesc,
    ...
    */
                        ...
                        for (int wch = 0; wch < channelsizeOfWeight; wch++)
                        {
                            for (int wro = 0; wro < rowsizeOfWeight; wro++)
                            {
                                for (int wco = 0; wco < colsizeOfWeight; wco++)
                                {
                                    ...
                                    result_index =
                                        Index5D(shapeOfResult, ti, ba, wch,
                                                (ro * m_stride[0]) + wro, (co * m_stride[1]) + wco);
    ...
    @details TransposedConvolution의 미분 값(weight * this_delta, input * this_delta)을 계산하여
    input_delta와 weight_gradient에 각각 더해 넣는다.
    ...
    */
    int ForwardPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnConvolutionBackwardData(
            this->GetCudnnHandle(), &m_alpha, filterDesc, m_pDevFilter, inputTensorDesc,
            m_pDevInput, convDesc, m_dataAlgo, m_dataDevWorkSpace, m_dataSizeInBytes, &m_beta,
            outputTensorDesc, m_pDevOutput));
    ...
    @details TransposedConvolution의 BackwardPropagate결과는 m_pDevInputDelta와 m_pDevFilterDelta에
    저장된다.
    @param pTime 연산 할 Tensor가 위치한 Time값.
    @return 성공 시 TRUE.
    */
    int BackPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnConvolutionForward(this->GetCudnnHandle(), &m_alpha, deltaDesc, m_pDevDelta,
                                           filterDesc, m_pDevFilter, convDesc, m_algo,
                                           m_devWorkSpace, m_sizeInBytes, &m_beta, inputDeltaDesc,
                                           m_pDevInputDelta));

        checkCUDNN(cudnnConvolutionBackwardFilter(
            this->GetCudnnHandle(), &m_alpha, deltaDesc, m_pDevDelta, inputTensorDesc, m_pDevInput,
            convDesc, m_filterAlgo, m_filterDevWorkSpace, m_filterSizeInBytes, &m_beta, filterDesc,
            m_pDevFilterDelta));
