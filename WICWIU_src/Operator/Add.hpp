...
class Addall : public Operator<DTYPE>
{
private:
...
#ifdef __CUDNN__
    cudnnTensorDescriptor_t leftTensorDesc, rightTensorDesc, outputTensorDesc, leftDeltaDesc,
        rightDeltaDesc, deltaDesc;
    ///< GPU내의 Tensor값들을 가르키기 위한 descriptor.
    DTYPE *m_pDevLeft, *m_pDevRight, *m_pDevOutput, *m_pDevLeftDelta, *m_pDevRightDelta,
        *m_pDevDelta;
...
public:
    ...
    */
    Addall(Operator<DTYPE>* pLeftInput, Operator<DTYPE>* pRightInput, std::string pName,
           int pLoadflag = TRUE)
        : Operator<DTYPE>(pLeftInput, pRightInput, pName, pLoadflag)
    {
#ifdef __DEBUG__
    ...
    */
    ...
    @details 파라미터로 들어온 pLeftInput과 m_pRightInput의 Shape정보를 맴버변수에 저장하고 다른
    맴버 변수들은 pLeftInput의 Shape값으로 초기화한다.
    ...
    */
    int Alloc(Operator<DTYPE>* pLeftInput, Operator<DTYPE>* pRightInput)
    {
        ...
        this->SetResult(
            new Tensor<DTYPE>(m_timesize, m_batchsize, m_channelsize, m_rowsize, m_colsize));
    ...
    */
    void Delete()
    {
        ...
        if (leftTensorDesc)
            checkCUDNN(cudnnDestroyTensorDescriptor(leftTensorDesc));
        leftTensorDesc = NULL;

        if (rightTensorDesc)
            checkCUDNN(cudnnDestroyTensorDescriptor(rightTensorDesc));
        ...
        if (leftDeltaDesc)
            checkCUDNN(cudnnDestroyTensorDescriptor(leftDeltaDesc));
        leftDeltaDesc = NULL;

        if (rightDeltaDesc)
            checkCUDNN(cudnnDestroyTensorDescriptor(rightDeltaDesc));
        rightDeltaDesc = NULL;
    ...
    */
    ...
    @details Container에 저장한 pLeftInput, pRightInput의 Gradient값에 계산한 Gradient값을 각각
    더한다.
    @param pTime 연산 할 Tensor가 위치한 Time값. default는 0으로 사용한다.
    @return 성공 시 TRUE.
    */
    ...
    */
    int ForwardPropagateOnGPU(int pTime)
    {
        ...
        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &m_alpha, leftTensorDesc, m_pDevLeft,
                                  &m_alpha, outputTensorDesc, m_pDevOutput));

        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &m_alpha, rightTensorDesc, m_pDevRight,
                                  &m_alpha, outputTensorDesc, m_pDevOutput));
    ...
    @details cudnnAddTensor를 이용히여 m_pDevDelta값을 m_pDevLeftDelta, m_pDevRightDelta에 각각
    더한다.
    @param pTime 연산 할 Tensor가 위치한 Time값.
    @return 성공 시 TRUE.
    */
    int BackPropagateOnGPU(int pTime)
    {
        ...
        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &m_alpha, deltaDesc, m_pDevDelta,
...
*/
template <typename DTYPE>
class AddColWise : public Operator<DTYPE>
{
private:
...
#ifdef __CUDNN__
    cudnnTensorDescriptor_t inputTensorDesc, biasTensorDesc, outputTensorDesc, inputDeltaDesc,
        biasDeltaDesc, deltaDesc;
    ///<  GPU내의 Tensor값들을 가르키기 위한 descriptor.
    DTYPE *m_pDevInput, *m_pDevBias, *m_pDevOutput, *m_pDevInputDelta, *m_pDevBiasDelta,
        *m_pDevDelta;
    ///<  cudnn 연산에서 사용 할 데이터를 가리키는 맴버 변수.
...
public:
    ...
    */
    AddColWise(Operator<DTYPE>* pInput, Operator<DTYPE>* pBias, std::string pName,
               int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pBias, pName, pLoadflag)
    {
#ifdef __DEBUG__
        std::cout
            << "AddColWise::AddColWise(Operator<DTYPE> *, Operator<DTYPE> *, std::string, int)"
            << '\n';
#endif // __DEBUG__
    ...
    */
    ...
    @details 파라미터로 들어온 pInput, pBias의 Shape정보를 맴버 변수에 저장하고 다른 맴버 변수들은
    pInput의 Shape값으로 초기화 한다.
    @details Result값과 gradient값을 저장 할 Tensor를 새로 만든다.
    @param 생성 할 Tensor의 Shape정보를 가진 Operator.
    ...
    */
    int Alloc(Operator<DTYPE>* pInput, Operator<DTYPE>* pBias)
    {
        ...
        if ((*m_pBiasTenShape)[0] != 1)
            printf("Receive invalid bias shape in %s (%s %d), cannot handling\n", __FUNCTION__,
                   __FILE__, __LINE__);

        if ((*m_pBiasTenShape)[1] != 1)
            printf("Receive invalid bias shape in %s (%s %d), cannot handling\n", __FUNCTION__,
                   __FILE__, __LINE__);

        if ((*m_pBiasTenShape)[2] != 1)
            printf("Receive invalid bias shape in %s (%s %d), cannot handling\n", __FUNCTION__,
                   __FILE__, __LINE__);

        if ((*m_pBiasTenShape)[3] != 1)
            printf("Receive invalid bias shape in %s (%s %d), cannot handling\n", __FUNCTION__,
                   __FILE__, __LINE__);
#endif // __DEBUG__
    ...
    */
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        checkCUDNN(cudnnSetTensor4dDescriptor(biasDeltaDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1,
                                              m_colsize, 1, 1));
    ...
    */
    void Delete()
    {
        ...
        if (biasDeltaDesc)
            checkCUDNN(cudnnDestroyTensorDescriptor(biasDeltaDesc));
        biasDeltaDesc = NULL;
    ...
    */
    ...
    @details Container에 저장한 pInput, pBias의 Gradient값에 계산을 통해 구한 gradient값을 각각
    더한다.
    @param pTime 연산 할 Tensor가 위치한 Time값. default는 0으로 사용한다.
    @return 성공 시 TRUE.
    */
    ...
    @details cudnnAddTensor를 이용히여  m_pDevInput, m_pDevBias의 값을 더하여 m_pDevOutput에
    저장한다.
    @param pTime 연산 할 Tensor가 위치한 Time값.
    @return 성공 시 TRUE.
    */
    int ForwardPropagateOnGPU(int pTime)
    {
        ...
        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &m_alpha, inputTensorDesc, m_pDevInput,
                                  &m_alpha, outputTensorDesc, m_pDevOutput));

        checkCUDNN(cudnnAddTensor(this->GetCudnnHandle(), &m_alpha, biasTensorDesc, m_pDevBias,
                                  &m_alpha, outputTensorDesc, m_pDevOutput));
    ...
    */
    int BackPropagateOnGPU(int pTime)
    {
        ...
        checkCUDNN(cudnnConvolutionBackwardBias(this->GetCudnnHandle(), &m_alpha, deltaDesc,
                                                m_pDevDelta, &m_alpha, biasDeltaDesc,
                                                m_pDevBiasDelta))
...
*/
template <typename DTYPE>
class AddChannelWise : public Operator<DTYPE>
{
private:
    Shape* m_pInputTenShape;
    ///< 더해질 Tensor의 Shape
...
public:
    ...
    @ref int Alloc(Operator<DTYPE> *pInput, Operator<DTYPE> *pBias)
    */
    AddChannelWise(Operator<DTYPE>* pInput, Operator<DTYPE>* pBias, std::string pName,
                   int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pBias, pName, pLoadflag)
    {
#ifdef __DEBUG__
        std::cout << "AddChannelWise::AddChannelWise(Operator<DTYPE> *, Operator<DTYPE> *, "
                     "std::string, int)"
                  << '\n';
#endif // __DEBUG__
        this->Alloc(pInput, pBias);
    ...
    */
    ...
    /*!
    @brief AddColWise의 BackPropagate 매소드.
    @details Container에 저장한 pInput, pBias의 Gradient값애 계산을 통해 구한 gradient값을 각각
    더한다.
    @param pTime 연산 할 Tensor가 위치한 Time값. default는 0으로 사용한다.
    @return 성공 시 TRUE.
    */
