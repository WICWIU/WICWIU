...
class GaussianNoiseGenerator : public NoiseGenerator<DTYPE>
{
private:
...
public:
    ...
    GaussianNoiseGenerator(int pTimeSize, int pBatchSize, int pChannelSize, int pRowSize,
                           int pColSize, float mean, float stddev,
                           IsTruncated pTruncated = NoUseTruncated, IsUseTime pAnswer = NoUseTime,
    ...
    GaussianNoiseGenerator(Shape* pShape, float mean, float stddev,
                           IsTruncated pTruncated = NoUseTruncated, IsUseTime pAnswer = NoUseTime,
                           std::string pName = "No Name")
        : NoiseGenerator<DTYPE>(pShape, pName)
    {
        m_mean = mean;
        m_stddev = stddev;
        m_isTruncated = pTruncated;
    ...
    GaussianNoiseGenerator(int pTimeSize, int pBatchSize, int pChannelSize, int pRowSize,
                           int pColSize, float mean, float stddev, float pTrunc,
                           IsTruncated pTruncated = NoUseTruncated, IsUseTime pAnswer = NoUseTime,
                           std::string pName = "No Name")
        : NoiseGenerator<DTYPE>(pTimeSize, pBatchSize, pChannelSize, pRowSize, pColSize, pName)
    {
    ...
    GaussianNoiseGenerator(Shape* pShape, float mean, float stddev, float pTrunc,
                           IsTruncated pTruncated = NoUseTruncated, IsUseTime pAnswer = NoUseTime,
                           std::string pName = "No Name")
        : NoiseGenerator<DTYPE>(pShape, pName)
    {
    ...
    int GenerateNoise()
    {
        ...
        do
        {
            ...
            {
                temp = Tensor<float>::Truncated_normal(m_timesize, m_batchsize, m_channelsize,
                                                       m_rowsize, m_colsize, m_mean, m_stddev,
                                                       m_trunc);
            }
            else
            {
                temp = Tensor<float>::Random_normal(m_timesize, m_batchsize, m_channelsize,
                                                    m_rowsize, m_colsize, m_mean, m_stddev);
            }
