...
public:
    BatchNormalize(Operator<DTYPE>* pInput, Operator<DTYPE>* pScale, Operator<DTYPE>* pBias,
                   int pIsChannelwise = TRUE, std::string pName = NULL, int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pScale, pBias, pName, pLoadflag)
    {
#if __DEBUG__
    ...
    BatchNormalize(Operator<DTYPE>* pInput, Operator<DTYPE>* pScale, Operator<DTYPE>* pBias,
                   int pIsChannelwise = TRUE, float pMomentum = 0.1, std::string pName = NULL,
                   int pLoadflag = TRUE)
        : Operator<DTYPE>(pInput, pScale, pBias, pName, pLoadflag)
    {
#if __DEBUG__
    ...
    int Alloc(Operator<DTYPE>* pInput, Operator<DTYPE>* pScale, Operator<DTYPE>* pBias,
              int pIsChannelwise, float pMomentum = 0.1, double pEpsilon = 0.01)
    {
#if __DEBUG__
        ...
        this->SetResult(new Tensor<DTYPE>(m_inputTimeSize, m_inputBatchSize, m_numChannel,
                                          m_numInputRow, m_numInputColumn));
        this->SetGradient(new Tensor<DTYPE>(m_inputTimeSize, m_inputBatchSize, m_numChannel,
                                            m_numInputRow, m_numInputColumn));
    ...
    void InitializeAttributeForGPU(unsigned int idOfDevice)
    {
        ...
        if (m_momentum != 0)
            m_CUDNNExponentialAverageFactor = m_momentum;
        else
            ...
            case TRAIN:
                ...
                if (m_momentum == 0)
                    m_CUDNNExponentialAverageFactor =
                        (m_CUDNNExponentialAverageFactor /
                         (m_CUDNNExponentialAverageFactor + 1)); // for exponential
                break;
            case ACCUMULATE:
                checkCUDNN(cudnnBatchNormalizationForwardTraining(
                    m_CUDNNHandle, m_CUDNNMode, &m_CUDNNAlpha, &m_CUDNNBeta, m_CUDNNXDesc, CUDNNX,
    ...
    int BackPropagateOnGPU(int pTime = 0)
    {
        ...
        checkCUDNN(cudnnBatchNormalizationBackward(
            this->GetCudnnHandle(), m_CUDNNMode, &m_CUDNNAlpha, &m_CUDNNAlpha, &m_CUDNNAlpha,
            &m_CUDNNAlpha, m_CUDNNXDesc, CUDNNX, m_CUDNNDyDesc, CUDNNDy, m_CUDNNDxDesc, CUDNNDx,
            m_CUDNNBatchSummaryDesc, CUDNNBnScale, CUDNNBnScaleDiff, CUDNNBnBiasDiff,
            m_CUDNNEpsilon, CUDNNCachedMean,
            CUDNNCachedInvVariance /* CUDNNCachedMean, CUDNNCachedInvVariance*/));
