...
FewShotClassifier::FewShotClassifier(int inputDim, int featureDim, const std::vector<std::string> vClassName, NeuralNetwork<float> *pNN, int noRef, int *pRefLabel, float *pRefSample[], int batchSize)
{
...
FewShotClassifier::FewShotClassifier(int inputDim, int featureDim, const std::vector<std::string> vClassName, NeuralNetwork<float> *pNN, KNearestNeighbor *kNN){
  for(int i = 0; i < vClassName.size(); i++)
    m_aClassName.push_back(vClassName[i]);
...
//  InputToFeature(int inDim, int noSample, float *pSamples[], int outDim, float *pFeatures[], int batchSize)
