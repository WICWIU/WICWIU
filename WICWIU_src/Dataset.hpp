...
public:
    ...
    virtual ~WData() { delete[] m_aData; }

    virtual DTYPE* GetData() { return m_aData; }

    virtual int GetCapacity() { return m_capacity; }

    DTYPE& operator[](int idx) { return m_aData[idx]; }
};
...
public:
    ...
    virtual int GetLabel(int idx)
    {
        if (idx < 0 || idx >= label.size())
        {
            printf("idx = %d is out of range (label.size() = %lu) in %s (%s %d)\n", idx,
                   label.size(), __FUNCTION__, __FILE__, __LINE__);
    ...
    virtual void CopyData(int idx, DTYPE* pDest)
    { // copy i-th iamge into pDest. (designed for k-NN)
        printf("This functions should be overriden by derived class");
        printf("Press Enter to continue... (%s)", __FUNCTION__);
        getchar();
    }

    virtual void SetPosNegIndices(std::vector<int>* pvPosIndex, std::vector<int>* pvNegIndex)
    { // registers indices fo positive and negative samples for each sample
    ...
    virtual int GetPositiveIndex(int idx)
    {                        // for triplet loss
    ...
    virtual int GetNegativeIndex(int idx)
    {                        // for triplet loss
        if (rand() % 2 != 0) // for stochasticity
            return -1;
...
template <typename DTYPE>
std::vector<Tensor<DTYPE>*>* Dataset<DTYPE>::GetData(int idx)
{
    // virtual
...
template <typename DTYPE>
int Dataset<DTYPE>::GetNumOfDatasetMember()
{
    // # of data columns
    // e.g. (X, Y) : returns 2
    // e.g. (X, Y, Z): returns 3
...
template <typename DTYPE>
void Dataset<DTYPE>::SetLabel(const int* pLabel, int noLabel)
{
    try
    {
...
template <typename DTYPE>
void Dataset<DTYPE>::SetLabel(const unsigned char* pLabel, int noLabel)
{
    try
    {
        label.resize(noLabel);
...
template <typename DTYPE>
std::vector<Tensor<DTYPE>*>* Dataset<DTYPE>::GetDataOfPositiveLabel(int anchorIdx, int* pPosIdx)
{
    ...
    if (anchorLabel < 0)
    {
        printf("Error! The label of anchor sample (idx = %d) is %d in %s (%s %d)\n", anchorIdx,
               anchorLabel, __FUNCTION__, __FILE__, __LINE__);
        return NULL;
    }
...
template <typename DTYPE>
std::vector<Tensor<DTYPE>*>* Dataset<DTYPE>::GetDataOfNegativeLabel(int anchorIdx, int* pNegIdx)
{
    std::vector<Tensor<DTYPE>*>* result = new std::vector<Tensor<DTYPE>*>(0, NULL);
